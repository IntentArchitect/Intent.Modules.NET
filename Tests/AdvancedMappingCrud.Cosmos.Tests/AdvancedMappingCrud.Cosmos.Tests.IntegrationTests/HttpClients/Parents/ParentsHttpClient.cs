...
namespace AdvancedMappingCrud.Cosmos.Tests.IntegrationTests.HttpClients.Parents
{
    public class ParentsHttpClient : IParentsService
    {
        ...
        public ParentsHttpClient(HttpClient httpClient)
        {
        ...
        public async Task<string> CreateParentAsync(
            CreateParentCommand command,
            CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/parents";
        ...
        public async Task DeleteParentAsync(string id, CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/parents/{id}";
        ...
        public async Task<ParentDto> GetParentByIdAsync(string id, CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/parents/{id}";
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<ParentDto>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }

        public async Task<List<ParentDto>> GetParentsAsync(CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/parents";
            using var httpRequest = new HttpRequestMessage(HttpMethod.Get, relativeUri);
            httpRequest.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(JSON_MEDIA_TYPE));
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<List<ParentDto>>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
