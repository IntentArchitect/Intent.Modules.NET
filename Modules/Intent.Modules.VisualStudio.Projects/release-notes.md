...
- New Feature: Added a `Central Package Floating Versions Enabled` property to the `Visual Studio Solution Options` stereotype. When enabled, `Directory.Packages.props` gains `CentralPackageFloatingVersionsEnabled` set to `true`, which is NuGet's opt-in for floating `PackageVersion` entries (e.g. `8.0.*`) under Central Package Management - without it, NuGet fails restore with `NU1011`.
