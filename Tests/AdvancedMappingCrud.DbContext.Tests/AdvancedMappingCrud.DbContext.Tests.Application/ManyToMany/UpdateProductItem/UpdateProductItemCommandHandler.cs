...
namespace AdvancedMappingCrud.DbContext.Tests.Application.ManyToMany.UpdateProductItem
{
    ...
    public class UpdateProductItemCommandHandler : IRequestHandler<UpdateProductItemCommand>
    {
        ...
        {
            var productItem = await _dbContext.ProductItems.SingleOrDefaultAsync(x => x.Id == request.Id, cancellationToken);
