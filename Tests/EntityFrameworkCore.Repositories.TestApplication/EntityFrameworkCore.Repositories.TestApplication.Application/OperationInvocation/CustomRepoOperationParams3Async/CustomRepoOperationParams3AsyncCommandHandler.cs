...
namespace EntityFrameworkCore.Repositories.TestApplication.Application.OperationInvocation.CustomRepoOperationParams3Async
{
    ...
    public class CustomRepoOperationParams3AsyncCommandHandler : IRequestHandler<CustomRepoOperationParams3AsyncCommand>
    {
        private readonly ICustomRepository _customRepository;

        [IntentManaged(Mode.Merge)]
        public CustomRepoOperationParams3AsyncCommandHandler(ICustomRepository customRepository)
        {
            _customRepository = customRepository;
        }

        [IntentManaged(Mode.Fully, Body = Mode.Fully)]
        public async Task Handle(CustomRepoOperationParams3AsyncCommand request, CancellationToken cancellationToken)
        {
            await _customRepository.Operation_Params3_ReturnsV_Collection0Async(
                new SpParameter(
