...
namespace EfCoreSoftDelete.IntegrationTests.HttpClients.Customers
{
    public class CustomersHttpClient : ICustomersService
    {
        ...
        public async Task<CustomerDto> GetDeletedCustomerByIdAsync(Guid id, CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/customers/deleted/{id}";
            var httpRequest = new HttpRequestMessage(HttpMethod.Get, relativeUri);
            httpRequest.Headers.Accept.Add(new MediaTypeWithQualityHeaderValue(JSON_MEDIA_TYPE));
