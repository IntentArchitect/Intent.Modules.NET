...
namespace Intent.Modules.AspNetCore.IntegrationTesting.Migrations
{
    public class OnInstallMigration : IModuleOnInstallMigration
    {
        ...
        private const string IntegrationTestsRoleName = "IntegrationTests";
        ...
        public void OnInstall()
        {
            ...
            foreach (var package in packages)
            {
                ...
                var integrationTestProject = AddIntegrationTestsProject(package, testFolder);
                AddIntegrationTestRole(package, integrationTestProject);
        ...
        private static IElementPersistable AddIntegrationTestsProject(IPackageModelPersistable package, IElementPersistable testFolder)
        {
            var applicationProjectSettings = GetApplicationProjectSettings(package);

            if (package.GetElementsOfType(CSharpProjectSpecializationId).All(p => p.Name != $"{applicationProjectSettings.ProjectName}.IntegrationTests"))
            {
                var testProject = package.Classes.Add(Guid.NewGuid().ToString(), CSharpProjectSpecializationType, CSharpProjectSpecializationId,
                    $"{applicationProjectSettings.ProjectName}.IntegrationTests", testFolder.Id);
            ...
            return package.GetElementsOfType(CSharpProjectSpecializationId).FirstOrDefault(p => p.Name == $"{applicationProjectSettings.ProjectName}.IntegrationTests");
        }

        private static void AddIntegrationTestRole(IPackageModelPersistable package, IElementPersistable integrationTestProject)
        {
            var integrationTestRole = package.GetElementsOfType(RoleSpecializationId).FirstOrDefault(r => r.Name == IntegrationTestsRoleName);

            if (integrationTestRole != null)
            {
                return;
            }

            integrationTestProject.ChildElements.Add(Guid.NewGuid().ToString(), RoleSpecializationType, RoleSpecializationId, IntegrationTestsRoleName, integrationTestProject.Id);
        }
