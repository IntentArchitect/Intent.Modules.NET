...
namespace WolverineEventing.Transport.AzureServiceBus.Infrastructure.Dispatch.Middleware
{
    public class PerformanceMiddleware
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogWarning("WolverineEventing.Transport.AzureServiceBus Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName} {@Request}", requestName, stopwatch.ElapsedMilliseconds, user?.Id, user?.Name, envelope.Message);
            }
            else
            {
                logger.LogWarning("WolverineEventing.Transport.AzureServiceBus Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName}", requestName, stopwatch.ElapsedMilliseconds, user?.Id, user?.Name);
            }
        }
    }
}