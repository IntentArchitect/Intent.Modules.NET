...
using Intent.Modules.Application.CQRS.CRUD.CrudStrategies;
...
namespace Intent.Modules.Application.CQRS.CRUD.FactoryExtensions
{
    ...
    public class CqrsHandlerCrudExtension : FactoryExtensionBase
    {
        ...
        private static void InstallOnCommandHandlers(IApplication application)
        {
            ...
            {
                var model = (template as ITemplateWithModel)?.Model as CommandModel;
                if (model == null)
                {
                    continue;
                }
        ...
        {
            var templates = application.FindTemplateInstances<ICSharpFileBuilderTemplate>(TemplateRoles.Application.Handler.Query);
            foreach (var template in templates)
            {
                var model = (template as ITemplateWithModel)?.Model as QueryModel;
                if (model == null)
                {
                    continue;
                }
