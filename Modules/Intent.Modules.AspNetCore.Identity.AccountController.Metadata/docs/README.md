...
## The routes here are fixed at `api`

The `Account` service in this package declares its routes as literal values: `api/[controller]/[action]` for most operations, and `~/api/[controller]/...` for `forgotPassword`, `resetPassword` and `manage/info`.

Those literals cannot track the **Default API Route Prefix** setting. The package is referenced read-only out of this module rather than copied into each consuming application — in a consuming `.pkg.config` it appears as `isExternal="true"` with a `<module>` element and no `<path>` — so a single shared copy of static XML serves every application, and there is no per-application copy a migration could rewrite.

From `Intent.AspNetCore.Identity.AccountController` 4.2.0 the generated controller _does_ follow that setting. If you change the prefix away from `api`, the proxies generated from this package will call URLs the controller no longer serves, and you will need to correct the generated clients by hand. The Software Factory logs a warning naming this whenever the resolved prefix is not `api`. Applications left on the default `api/` are unaffected.
