...
namespace AdvancedMappingCrud.Repositories.Tests.IntegrationTests.Tests.Basics
{
    ...
    public class DeleteBasicTests : BaseIntegrationTest
    {
        ...
        public async Task DeleteBasic_ShouldDeleteBasic()
        {
            ...
            // Act
            await client.DeleteBasicAsync(basicId, TestContext.Current.CancellationToken);

            // Assert
            var exception = await Assert.ThrowsAsync<HttpClientRequestException>(() => client.GetBasicByIdAsync(basicId, TestContext.Current.CancellationToken));
