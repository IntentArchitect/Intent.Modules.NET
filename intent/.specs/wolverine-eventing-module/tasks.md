...
- [x] T0. Prerequisites — module creation, version increments, probes
  - [x] T0.1 [module] (satisfies: R18) — Create the `Intent.Wolverine.Common` module package
  ...
  - [x] T0.2 [module] (satisfies: R14.1) — Create the `Intent.Eventing.Wolverine` module package
  ...
  - [x] T0.3 [module] (satisfies: R18.3) — Increment `Intent.Application.Wolverine` to `1.1.0-pre.0`
  ...
  - [x] T0.4 [code] (satisfies: R7.1, R12.2, R2.6) — Extend the D6 compile-only probe with the four uncited call shapes
...
- [x] T1. Shared Wolverine host module (R18)
  - [x] T1.1 [model] (satisfies: R18.2) — Add the `WolverineFx` NuGet Package to `Intent.Wolverine.Common`
    - `NuGet Package` element with a `Package Version` child pinned to `5.39.5`, Minimum Target Framework `.NETCoreApp,Version=v8.0`
  - [x] T1.2 [model] (satisfies: R18.2) — Add the `WolverineHostRegistrationExtension` Factory Extension element
    - Factory Extension in `Intent.Wolverine.Common`; owns the single `ConfigureHostBuilderChainStatement("UseWolverine", ...)` call
  - [x] T1.3 [code] (satisfies: R18.2) — Write the broadcast request type in the module's API namespace
  ...
  - [x] T1.4 [code] (satisfies: R18.2, R8.2) — Implement `WolverineHostRegistrationExtension`
  ...
  - [x] T1.5 [code] (satisfies: R18.3, R18.6) — Rework `WolverineRegistrationFactoryExtension` in `Intent.Application.Wolverine`
  ...
  - [x] T1.6 [code] (satisfies: R18.6) — Marker sweep on the Golden Sample
  ...
  - [x] T1.7 [code] (satisfies: R18.3) — Docs for both host-side modules
...
- [x] T2. Provider foundation and designation (R1, R9)
  - [x] T2.1 [model] (satisfies: R9.1) — Add the `Wolverine Message` stereotype definition
  ...
  - [x] T2.2 [model] (satisfies: R1.1, R9.1, R9.2, R9.3) — Add the `Wolverine` Message Bus Provider element
  ...
  - [x] T2.3 [code] (satisfies: R1.1, R1.2, R1.6) — Model-reading seam shared by the module's templates
  ...
  - [x] T2.4 [model] (satisfies: R9.6) — Add the `WolverineMessageBusInteropExtension` Factory Extension element
  - [x] T2.5 [code] (satisfies: R9.4, R9.5, R9.6) — Designation filtering and Composite Message Bus participation
...
- [x] T3. Transport selection and broker topology (R2, R10)
  - [x] T3.1 [model] (satisfies: R14.2, R2.1, R2.7) — Add the `Wolverine Message Bus Settings` Module Settings Configuration with its Transport and Broker Topology fields
  ...
  - [x] T3.2 [model] (satisfies: R10.1, R10.3, R10.5, R2.2) — Add the seven `NuGet Package` elements
  ...
  - [x] T3.3 [model] (satisfies: R2.1, R2.3) — Add the `WolverineEventingConfiguration` template and `WolverineEventingRegistrationExtension` elements
  ...
  - [x] T3.4 [code] (satisfies: R2.1, R2.2, R2.4, R2.5) — Transport branch in `WolverineEventingConfiguration`
  ...
  - [x] T3.5 [code] (satisfies: R2.3) — Broadcast the host-configuration request and raise the appsettings registrations
  ...
  - [x] T3.6 [code] (satisfies: R2.6) — Fail-fast configuration read for Azure Service Bus and Amazon SQS
  ...
  - [x] T3.7 [code] (satisfies: R2.7) — Broker Topology emission
    - `Auto-provision` emits provisioning; `Externally owned` emits none and never declares exchanges, queues or bindings
  - [x] T3.8 [code] (satisfies: R14.2) — Document the settings surface and the transport matrix in `docs/README.md` and release notes

- [x] T4. Publishing and sending (R3, R4)
  - [x] T4.1 [model] (satisfies: R3.3, R3.4, R3.5, R3.6) — Add the `Message Topology Settings` stereotype definition
  ...
  - [x] T4.2 [model] (satisfies: R4.3, R4.4, R4.5, R4.6) — Add the `Command Distribution` stereotype definition
  ...
  - [x] T4.3 [model] (satisfies: R3.8, R8.8) — Add the `WolverineMessageBus` template element
    - C# Template, C# File Builder, Single File; Role `Infrastructure.Eventing.WolverineEventBus`, Default Location `Eventing`
  - [x] T4.4 [code] (satisfies: R3.1, R3.2, R3.10) — Publish-rule emission with the kebab-cased convention name
  - [x] T4.5 [code] (satisfies: R3.3, R3.4, R3.5, R3.6) — `Topic Name` override resolution and its `Validate Function`
  ...
  - [x] T4.6 [code] (satisfies: R4.1, R4.2, R4.7) — Send-rule emission and destination-queue resolution
  ...
  - [x] T4.7 [code] (satisfies: R3.8, R4.1, R8.8) — Implement `WolverineMessageBus`
  ...
  - [x] T4.8 [code] (satisfies: R3.9) — Conditional DI registration for the bus in `WolverineEventingRegistrationExtension`

- [x] T5. Consuming messages (R5)
  - [x] T5.1 [code] (satisfies: R5.1, R5.2) — Listener emission per subscribed message
  ...
  - [x] T5.2 [code] (satisfies: R5.3) — Subscriber Queue naming derivation
    - Derived, never a module setting (R14.2)
  - [x] T5.3 [code] (satisfies: R5.7, R5.9) — Exchange-to-queue binding emission
    - Emitted only under `Auto-provision`; suppressed under `Externally owned`
  - [x] T5.4 [code] (satisfies: R5.8) — Host-scope placement of the error handling policy hook
    - Reserve the seam that T6's policy emission writes into; policies are host-scope, not per-listener
  - [x] T5.5 [code] (satisfies: R5.4) — Confirm no competition with the `Intent.Eventing.Contracts` handler template
...
- [x] T6. Transactional Outbox and Error Handling Policy (R6, R7)
  - [x] T6.1 [model] (satisfies: R6.1) — Add the `Transactional Outbox` settings field
  ...
  - [x] T6.2 [model] (satisfies: R7.1) — Add the `Error Handling Policy` settings field
    - Select, Is Required true, Default `Retry with cooldown`, options `None` / `Retry` / `Retry with cooldown` / `Schedule retry`
  - [x] T6.3 [code] (satisfies: R6.1, R6.3) — Durability registration emission
  ...
  - [x] T6.4 [code] (satisfies: R6.5, R6.7) — Derive the durable storage technology from the modelled Database Provider
  - [x] T6.5 [code] (satisfies: R6.4, R6.8) — Stopping conditions as `FriendlyException`
  ...
  - [x] T6.6 [code] (satisfies: R6.3) — `eventbus-flush` tag handling in `WolverineMessageBusInteropExtension`
    - Applies only when a durable outbox is selected
  - [x] T6.7 [code] (satisfies: R7.1, R7.3) — Emit the `None`, `Retry` and `Retry with cooldown` policies
    - Uses the call shapes proven by T0.4; every policy terminates in move-to-error-queue
  - [x] T6.8 [code] (satisfies: R7.1, R7.4, R7.5) — Emit the `Schedule retry` policy and the empty-delay branch
    - Uses the call shape proven by T0.4; also terminates in move-to-error-queue
  - [x] T6.9 [code] (satisfies: R7.2) — Appsettings keys for the retry configuration
    - Raised as registration requests from `WolverineEventingRegistrationExtension`
  - [x] T6.10 [code] (satisfies: R6.1, R7.1) — Document outbox and error handling in `docs/README.md` and release notes

- [x] T7. Coexistence, telemetry and multi-tenancy (R8, R11, R12)
  - [x] T7.1 [code] (satisfies: R8.1, R8.3, R8.4, R8.5) — Type resolution across module boundaries
  ...
  - [x] T7.2 [model] (satisfies: R11.1) — Add the `WolverineTelemetryConfiguratorExtension` Factory Extension element
  - [x] T7.3 [code] (satisfies: R11.1) — Conditional `AddSource("Wolverine")` registration
    - Emits only when `Intent.OpenTelemetry` is installed; emits nothing otherwise
  - [x] T7.4 [model] (satisfies: R12.2) — Add the `WolverineTenantHeaderStrategy`, `WolverineTenantMiddleware` and `WolverineFinbuckleConfiguratorExtension` elements
    - Two C# Templates plus one Factory Extension; emitted only when the Finbuckle multi-tenancy module is installed
  - [x] T7.5 [code] (satisfies: R12.2) — Implement the tenant header strategy
    - Reads and writes the Tenant Identifier header on inbound and outbound messages
  - [x] T7.6 [code] (satisfies: R12.2) — Implement the tenancy middleware and its conditional wiring
...
- [x] T8. Settings close-out, migration guidance and module docs (R13, R14)
  - [x] T8.1 [code] (satisfies: R14.5) — Uninstall behaviour and the additive-appsettings caveat
  ...
  - [x] T8.2 [code] (satisfies: R13.1, R13.3, R13.5) — Write the MassTransit-to-Wolverine migration guidance
  ...
  - [x] T8.3 [code] (satisfies: R14.1, R14.2) — Final module docs and `CONTEXT.md` for `Intent.Eventing.Wolverine`
...
- [x] T9. Test Applications — transports and topology (R19)
  - WITHDRAWN (2026-08-26): T9.1 (`Wolverine.Transport.Local`) and T9.5 (`Wolverine.Topology.ExternallyOwned`) were removed from R19.1's matrix — neither application was ever built. See the requirements' `withdrawn-test-apps` callout for the coverage this gives up.
  - [x] T9.2 [model] (satisfies: R19.1) — `Wolverine.Transport.RabbitMQ.Publish` and `.Subscribe`
  ...
  - [x] T9.3 [model] (satisfies: R19.1) — `Wolverine.Transport.AzureServiceBus`
  - [x] T9.4 [model] (satisfies: R19.1) — `Wolverine.Transport.AmazonSqs`
  - [x] T9.6 [code] (satisfies: R19.1) — READMEs, committed generated output and clean re-run confirmation for T9.2 to T9.4
    ...
    - CLOSED (2026-08-25): READMEs written for all 6 apps; real `dotnet build` confirmed 0 errors for all 6; traceability recorded for T9.1-T9.6 against R19.1

- [x] T10. Test Applications — durability and error policies (R19)
  - [x] T10.1 [model] (satisfies: R19.1) — `Wolverine.Outbox.SqlServer.Publish` and `.Subscribe`
  - WITHDRAWN (2026-08-26): T10.2 (`Wolverine.Outbox.Postgresql.Publish` / `.Subscribe`) was removed from R19.1's matrix — never built, so the durable outbox is covered on SQL Server only.
  - [x] T10.3 [model] (satisfies: R19.1) — `Wolverine.ErrorPolicy.None`, `.Retry`, `.RetryWithCooldown` and `.ScheduleRetry`
    - One per Error Handling Policy, matching the four MassTransit retry-policy apps
  - [x] T10.4 [code] (satisfies: R19.1) — READMEs, committed generated output and clean re-run confirmation for T10.1 to T10.3
    - Same four-point re-run check as T9.6
    - CLOSED (2026-08-25): READMEs already existed for all 8 apps; traceability gap (5 of 8 apps missing) fixed — all 8 now recorded against R19.1

- [x] T11. Test Applications — coexistence, multi-provider and multi-tenancy (R19)
  - [x] T11.1 [model] (satisfies: R19.1, R18.6) — `Wolverine.Coexist.Cqrs` — eventing plus `Intent.Application.Wolverine` on one shared host
  ...
  - [x] T11.2 [model] (satisfies: R19.1, R9.4, R9.5) — `Wolverine.MultiProvider` — Wolverine alongside MassTransit, messages designated to each
    - Sole evidence for R9, and the app that has to demonstrate design assumption d-a2 holds
  - [x] T11.3 [model] (satisfies: R19.1, R12.2) — `Wolverine.MultiTenancy` — eventing plus the Finbuckle multi-tenancy module
    - Sole evidence for R12
  - [x] T11.4 [code] (satisfies: R19.1) — READMEs, committed generated output and clean re-run confirmation for T11.1 to T11.3
    - Same four-point re-run check as T9.6
