...
{
    public class SingleIndexEntityMapping : IMongoMappingConfiguration<SingleIndexEntity>
    {
        public string CollectionName => "SingleIndexEntities";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(SingleIndexEntity)))
            {
                BsonClassMap.RegisterClassMap<SingleIndexEntity>(
                    mapping =>
                    {
                        mapping.AutoMap();
