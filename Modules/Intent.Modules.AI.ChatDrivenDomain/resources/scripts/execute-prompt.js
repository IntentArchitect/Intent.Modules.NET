async function executePrompt(element) {
    ...
    const currentElements = lookupTypesOf("Class").map(clazz => ({
        ...
        typeReference: null, // Classes don't have typeReference
        children: [
    ...
    const currentAssociations = lookupTypesOf("Class").flatMap(clazz => clazz.getAssociations()
...
async function applyAiModelChanges(aiElements, aiAssociations, targetPackage) {
...
async function syncClass(aiClass, currentClasses, idMapping, targetPackage) {
...
async function syncAttributes(aiClass, intentClass, idMapping) {
    var _a;
    const currentAttributes = intentClass.getChildren("Attribute");
    // Find attributes in AI model
    const aiAttributes = ((_a = aiClass.children) === null || _a === void 0 ? void 0 : _a.filter((child) => child.specialization === "Attribute")) || [];
    for (const aiAttr of aiAttributes) {
        let intentAttr = currentAttributes.find((attr) => attr.id === aiAttr.id);
        if (!intentAttr) {
...
async function syncAssociations(aiAssociations, idMapping) {
    ...
    for (const aiAssoc of aiAssociations) {
        ...
        const existingAssoc = sourceClass.getAssociations().find(assoc => assoc.typeReference.getTypeId() === mappedTargetId &&
