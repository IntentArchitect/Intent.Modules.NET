...
A subscribed Integration Event listens on a queue named `{application-name-kebab}-{message-name-kebab}` — the subscribing application's name and the message's name, each kebab-cased and joined by a hyphen (e.g. `warehouse-order-created-event`). This name is **not overridable** in this release. A platform team that pre-provisions broker infrastructure needs this rule to create the right queue ahead of time for every subscribing application.
