...
namespace Wolverine.AzureFunctions.Infrastructure.Dispatch.Middleware
{
    public class PerformanceMiddleware
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogWarning("Wolverine.AzureFunctions Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@Request}", requestName, stopwatch.ElapsedMilliseconds, envelope.Message);
            }
            else
            {
                logger.LogWarning("Wolverine.AzureFunctions Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds)", requestName, stopwatch.ElapsedMilliseconds);
            }
        }
    }
}