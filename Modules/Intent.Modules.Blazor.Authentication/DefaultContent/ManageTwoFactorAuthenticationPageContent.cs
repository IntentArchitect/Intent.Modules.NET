        ...
        private static string BuildMudBlazorContent(string identityClass, string userAccessor, string redirectManager)
        {
            return $$"""
                ...
                <MudCard Class="ux-fade-in-up auth-form-shell" Style="animation-delay: 0.1s" Outlined="true">
                    <MudCardContent>
                        <MudText Typo="Typo.h5" Class="mb-3">Two-factor authentication (2FA)</MudText>
                        @if (canTrack)
                        {
                            if (is2faEnabled)
                            {
                                ...
                                <div class="two-fa-actions mb-4">
                                    ...
                                    {
                                        <form class="two-fa-inline-form" @formname="forget-browser" @onsubmit="OnSubmitForgetBrowserAsync" method="post">
                                            <AntiforgeryToken />
                                            <MudButton ButtonType="ButtonType.Submit" Variant="Variant.Outlined" Color="Color.Primary">Forget this browser</MudButton>
                                        </form>
                                    }
                                    <MudButton Href="Account/Manage/Disable2fa" Variant="Variant.Outlined" Color="Color.Primary">Disable 2FA</MudButton>
                                    <MudButton Href="Account/Manage/GenerateRecoveryCodes" Variant="Variant.Outlined" Color="Color.Primary">Reset recovery codes</MudButton>
                                </div>
                            }
                            ...
                            <div class="two-fa-actions mt-3">
                                @if (!hasAuthenticator)
                                {
                                    <MudButton Href="Account/Manage/EnableAuthenticator" Variant="Variant.Filled" Color="Color.Primary">Add authenticator app</MudButton>
                                }
                                else
                                {
                                    <MudButton Href="Account/Manage/EnableAuthenticator" Variant="Variant.Filled" Color="Color.Primary">Set up authenticator app</MudButton>
                                    <MudButton Href="Account/Manage/ResetAuthenticator" Variant="Variant.Outlined" Color="Color.Primary">Reset authenticator app</MudButton>
                                }
                            </div>
                        }
                        else
                ...
                @if (canTrack)
                {
                    if (is2faEnabled)
                    {
                        ...
                        <div class="ux-button-row">
                            ...
                            <a href="Account/Manage/Disable2fa" class="btn btn-danger"><UxIcon Name="shield-off" /> Disable 2FA</a>
                            <a href="Account/Manage/GenerateRecoveryCodes" class="btn btn-primary"><UxIcon Name="key" /> Reset recovery codes</a>
                        </div>
                    }
                    ...
                    <div class="ux-button-row">
                        @if (!hasAuthenticator)
                        {
                            <a href="Account/Manage/EnableAuthenticator" class="btn btn-primary"><UxIcon Name="shield" /> Add authenticator app</a>
                        }
                        else
                        {
                            <a href="Account/Manage/EnableAuthenticator" class="btn btn-primary"><UxIcon Name="shield" /> Set up authenticator app</a>
                            <a href="Account/Manage/ResetAuthenticator" class="btn btn-primary"><UxIcon Name="key" /> Reset authenticator app</a>
                        }
                    </div>
                }
                else
