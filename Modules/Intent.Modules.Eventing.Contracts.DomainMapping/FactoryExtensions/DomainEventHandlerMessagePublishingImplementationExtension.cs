...
namespace Intent.Modules.Eventing.Contracts.DomainMapping.FactoryExtensions
{
    ...
    public class DomainEventHandlerMessagePublishingImplementationExtension : FactoryExtensionBase
    {
        ...
        protected override void OnAfterTemplateRegistrations(IApplication application)
        {
            ...
            {
                var template = application.FindTemplateInstance<ICSharpFileBuilderTemplate>(TemplateFulfillingRoles.Application.DomainEventHandler.OldDefault, messageModel.GetMapFromDomainMapping().ElementId)
                    ?? application.FindTemplateInstance<ICSharpFileBuilderTemplate>(TemplateFulfillingRoles.Application.DomainEventHandler.Default, messageModel.GetMapFromDomainMapping().ElementId);
                if (template != null)
                {
