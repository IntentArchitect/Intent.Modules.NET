...
public class LocalCommandQueryMappingResolver : IMappingTypeResolver
{
    ...
    public ICSharpMapping? ResolveMappings(MappingModel mappingModel)
    {
        ...
        {
            if (templateInstance.CSharpFile.Classes.FirstOrDefault(x => x.RepresentedModel?.Id == mappingModel.Model.Id)?.Constructors.Any(x => x.Parameters.Count > 0) != true)
            {
                return new ParameterlessConstructoir(mappingModel, _template, templateInstance);
            }
            else
            {
    ...
    private class ParameterlessConstructoir(MappingModel model, ICSharpTemplate template, ICSharpFileBuilderTemplate commandOrQuery) : CSharpMappingBase(model, template)
    {        
