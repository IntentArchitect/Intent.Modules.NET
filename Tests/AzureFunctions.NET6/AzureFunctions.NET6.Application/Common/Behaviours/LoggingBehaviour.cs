...
namespace AzureFunctions.NET6.Application.Common.Behaviours
{
        ...
        where TRequest : notnull
    {
        ...
        public async Task Process(TRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogInformation("AzureFunctions.NET6 Request: {Name} {@UserId} {@UserName} {@Request}", requestName, user?.Id, user?.Name, request);
            }
            else
            {
                _logger.LogInformation("AzureFunctions.NET6 Request: {Name} {@UserId} {@UserName}", requestName, user?.Id, user?.Name);
            }
        }
    }
}