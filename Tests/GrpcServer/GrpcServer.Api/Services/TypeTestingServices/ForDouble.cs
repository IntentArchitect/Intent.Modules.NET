...
{
    [Authorize]
    public class ForDouble : Protos.Services.TypeTestingServices.ForDouble.ForDoubleBase
    {
        private readonly IForDoubleService _appService;
        private readonly IEventBus _eventBus;
        private readonly IUnitOfWork _unitOfWork;

        public ForDouble(IForDoubleService appService, IEventBus eventBus, IUnitOfWork unitOfWork)
        {
        ...
        public override async Task<DoubleValue> Operation(ForDoubleOperationRequest request, ServerCallContext context)
        {
            double result;
            ...
            return new DoubleValue { Value = result };
        }

        public override async Task<ListOfDouble> OperationCollection(
            ForDoubleOperationCollectionRequest request,
            ServerCallContext context)
        {
            ...
            await _eventBus.FlushAllAsync(context.CancellationToken);
            return ListOfDouble.Create(result);
        }

        public override async Task<DoubleValue?> OperationNullable(
            ForDoubleOperationNullableRequest request,
            ServerCallContext context)
        {
            double? result;
            ...
            await _eventBus.FlushAllAsync(context.CancellationToken);
            return result.HasValue ? new DoubleValue { Value = result.Value } : null;
        }

        public override async Task<ListOfDouble?> OperationNullableCollection(
            ForDoubleOperationNullableCollectionRequest request,
            ServerCallContext context)
        {
            List<double>? result;
