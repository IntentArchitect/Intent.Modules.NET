...
namespace Wolverine.AzureFunctions.Infrastructure.Dispatch.Middleware
{
    public class LoggingMiddleware
    {
        ...
        private async Task LogAsync(object request, ILogger logger)
        {
            ...
            {
                logger.LogInformation("Wolverine.AzureFunctions Request: {Name} {@Request}", requestName, request);
            }
            else
            {
                logger.LogInformation("Wolverine.AzureFunctions Request: {Name}", requestName);
            }
        }
    }
}