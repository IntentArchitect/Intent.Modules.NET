...
namespace EntityFrameworkCore.CosmosDb.TestApplication.Infrastructure.Repositories.InheritanceAssociations
{
    ...
    {
        public DerivedClassForAbstractRepository(ApplicationDbContext dbContext, IMapper mapper) : base(dbContext, mapper)
        {
        }
        ...
        public async Task<DerivedClassForAbstract?> FindByIdAsync(
            Guid id,
            Func<IQueryable<DerivedClassForAbstract>, IQueryable<DerivedClassForAbstract>> queryOptions,
            CancellationToken cancellationToken = default)
        {
