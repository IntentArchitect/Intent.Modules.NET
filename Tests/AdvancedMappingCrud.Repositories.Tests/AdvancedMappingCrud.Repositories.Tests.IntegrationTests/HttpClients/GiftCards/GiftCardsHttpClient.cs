...
{
    public class GiftCardsHttpClient : IGiftCardsService
    {
        private const string JSON_MEDIA_TYPE = "application/json";
        private readonly HttpClient _httpClient;
        private readonly JsonSerializerOptions _serializerOptions;

        public GiftCardsHttpClient(HttpClient httpClient)
        {
        ...
        public async Task<string> CreateGiftCardAsync(
            string cardCode,
            CreateGiftCardCommand command,
            CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/gift-cards/{cardCode}";
        ...
        public async Task DeleteGiftCardAsync(string cardCode, CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/gift-cards/{cardCode}";
        ...
        public async Task<GiftCardDto> GetGiftCardByIdAsync(string cardCode, CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/gift-cards/{cardCode}";
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<GiftCardDto>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
