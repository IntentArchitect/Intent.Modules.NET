...
{
    public class MapMapMeMapping : IMongoMappingConfiguration<MapMapMe>
    {
        public string CollectionName => "MapMapMes";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(MapMapMe)))
            {
                BsonClassMap.RegisterClassMap<MapMapMe>(
                    mapping =>
                    {
                        mapping.AutoMap();
