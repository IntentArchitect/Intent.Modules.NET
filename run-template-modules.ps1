...
$intent_solution = 'Modules/Intent.Modules.NET.isln'
...
$block = { Param($application, $intent_architect_user, $intent_architect_password, $intent_solution, $writeOutput)
	...
	$output = intent-cli "ensure-no-outstanding-changes" "--application-id" "$($appId)" "--check-deviations" "--" "$($intent_architect_user)" "$($intent_architect_password)" "$($intent_solution)"
