        ...
        where TRequest : notnull
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (elapsedMilliseconds > 500)
            {
                ...
                {
                    _logger.LogWarning("AspNetCore.AzureServiceBus.GroupB Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName} {@Request}", requestName, elapsedMilliseconds, userId, userName, request);
                }
                else
                {
                    _logger.LogWarning("AspNetCore.AzureServiceBus.GroupB Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName}", requestName, elapsedMilliseconds, userId, userName);
                }
            }
            return response;
        }
