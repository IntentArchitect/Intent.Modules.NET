...
{
    public class MapAggPeerMapping : IMongoMappingConfiguration<MapAggPeer>
    {
        public string CollectionName => "MapAggPeers";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(MapAggPeer)))
            {
                BsonClassMap.RegisterClassMap<MapAggPeer>(
                    mapping =>
                    {
                        mapping.AutoMap();
