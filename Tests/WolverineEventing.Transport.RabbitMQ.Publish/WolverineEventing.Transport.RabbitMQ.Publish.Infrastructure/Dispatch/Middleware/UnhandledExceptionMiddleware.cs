...
namespace WolverineEventing.Transport.RabbitMQ.Publish.Infrastructure.Dispatch.Middleware
{
    public class UnhandledExceptionMiddleware
    {
        ...
        private void LogException(Exception exception, object? request, ILogger logger)
        {
            ...
            {
                logger.LogError(exception, "WolverineEventing.Transport.RabbitMQ.Publish Request: Unhandled Exception for Request {Name} {@Request}", requestName, request);
            }
            else
            {
                logger.LogError(exception, "WolverineEventing.Transport.RabbitMQ.Publish Request: Unhandled Exception for Request {Name}", requestName);
            }
        }
    }
}