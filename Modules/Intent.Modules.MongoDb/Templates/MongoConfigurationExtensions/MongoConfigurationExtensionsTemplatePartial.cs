            ...
            CSharpFile = new CSharpFile(this.GetNamespace(), this.GetFolderPath())
                ...
                .AddClass($"MongoConfigurationExtensions", @class =>
                {
                    ...
                    @class.AddMethod("IServiceCollection", "AddMongoCollection<T>", addMongoCollection =>
                    {
                        ...
                        // Registered as Scoped (not Singleton) because IMongoDatabase is resolved per-tenant
                        // (Scoped) in multi-tenant applications - see the application's tenant-aware
                        // IMongoDatabase registration in its DI setup. A Singleton collection would capture
                        // only the first tenant's database and/or throw "Cannot resolve scoped service
                        // 'IMongoDatabase' from root provider" at runtime.
                        addMongoCollection.AddStatements($@"services.AddScoped(sp =>
                        {{
