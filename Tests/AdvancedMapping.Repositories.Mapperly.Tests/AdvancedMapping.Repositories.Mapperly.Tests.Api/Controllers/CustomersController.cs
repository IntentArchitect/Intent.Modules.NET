...
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.CreateCustomer;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.DeleteCustomer;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.GetCustomerById;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.GetCustomers;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.GetCustomersLean;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.GetCustomersPaged;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.GetCustomersPagedMapped;
using AdvancedMapping.Repositories.Mapperly.Tests.Application.Customers.UpdateCustomer;
...
namespace AdvancedMapping.Repositories.Mapperly.Tests.Api.Controllers
{
    ...
    public class CustomersController : ControllerBase
    {
        ...
        /// <summary>
        /// </summary>
        /// <response code="200">Returns the specified List&lt;CustomerLeanDto&gt;.</response>
        [HttpGet("api/customers/lean")]
        [ProducesResponseType(typeof(List<CustomerLeanDto>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<List<CustomerLeanDto>>> GetCustomersLean(CancellationToken cancellationToken = default)
        {
            var result = await _mediator.Send(new GetCustomersLeanQuery(), cancellationToken);
            return Ok(result);
        }
        ...
        /// <response code="400">One or more validation errors have occurred.</response>
        [HttpGet("api/customers/paged-mapped")]
        [Produces(MediaTypeNames.Application.Json)]
        [ProducesResponseType(typeof(PagedResult<CustomerSummaryDto>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<PagedResult<CustomerSummaryDto>>> GetCustomersPagedMapped(
            [FromQuery][Required] int pageNo,
            [FromQuery][Required] int pageSize,
            [FromQuery] string? orderBy,
            CancellationToken cancellationToken = default)
        {
            var result = await _mediator.Send(new GetCustomersPagedMappedQuery(pageNo: pageNo, pageSize: pageSize, orderBy: orderBy), cancellationToken);
            return Ok(result);
        }
