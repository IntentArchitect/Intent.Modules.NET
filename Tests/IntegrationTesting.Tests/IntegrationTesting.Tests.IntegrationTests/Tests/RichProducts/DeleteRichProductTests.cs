...
namespace IntegrationTesting.Tests.IntegrationTests.Tests.RichProducts
{
    ...
    public class DeleteRichProductTests : BaseIntegrationTest
    {
        ...
        public async Task DeleteRichProduct_ShouldDeleteRichProduct()
        {
            ...
            // Act
            await client.DeleteRichProductAsync(richProductId, TestContext.Current.CancellationToken);

            // Assert
            var exception = await Assert.ThrowsAsync<HttpClientRequestException>(() => client.GetRichProductByIdAsync(richProductId, TestContext.Current.CancellationToken));
