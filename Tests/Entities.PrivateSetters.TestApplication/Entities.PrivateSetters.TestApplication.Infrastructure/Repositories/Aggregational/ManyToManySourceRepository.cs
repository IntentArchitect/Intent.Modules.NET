...
namespace Entities.PrivateSetters.TestApplication.Infrastructure.Repositories.Aggregational
{
    ...
    public class ManyToManySourceRepository : RepositoryBase<ManyToManySource, ManyToManySource, ApplicationDbContext>, IManyToManySourceRepository
    {
        ...
        public async Task<ManyToManySource?> FindByIdAsync(Guid id, CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.Id == id, cancellationToken);
        }

        public async Task<ManyToManySource?> FindByIdAsync(
            Guid id,
            Func<IQueryable<ManyToManySource>, IQueryable<ManyToManySource>> queryOptions,
            CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.Id == id, queryOptions, cancellationToken);
        }

        public async Task<List<ManyToManySource>> FindByIdsAsync(Guid[] ids, CancellationToken cancellationToken = default)
        {
