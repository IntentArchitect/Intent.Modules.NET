...
namespace CompositePublishTest.Infrastructure.Eventing.Kafka
{
        ...
        where TValue : class
    {
            ...
            ILogger<KafkaProducer<TKey, TValue>> logger)
        {
            ...
            // Allow overriding config per message type
            ...
            {
                logger.LogInformation("Using message type specific configuration for {MessageType}", typeof(TValue).Name);
            }
            else
            {
                producerConfig = new ProducerConfig(defaultProducerConfig);
            }

            // Topic resolution: Argument -> Config -> Type Name
            _topic = topicName 
                     ?? configuration[$"Kafka:MessageTypes:{messageType}:Topic"] 
                     ?? typeof(TValue).Name;
