...
- **UJ-6. Rashid joins a broker estate that already has its own names.**
  ...
  - **Path:** He sets a Topic Name override on each Integration Event and a Destination Queue Name override on each Integration Command, each on the element itself; re-runs the Software Factory; checks the generated destinations against the platform team's list.
...
- **`Wolverine Message` stereotype** — the one and only stereotype this module defines, per R9.8. It plays two roles at once: it is the applicable stereotype that makes Wolverine selectable as an Eventing Provider, and it carries the optional Topic Name and Destination Queue Name overrides. Applied automatically by the Message Bus stereotype when Wolverine is designated; applied by hand only when an override is wanted.
...
- **Topic Name** — the broker destination an Integration Event is published to. Derived by convention from the message type, optionally overridden per Integration Event through the `Wolverine Message` stereotype.
- **Destination Queue Name** — the broker queue an Integration Command is sent to. Derived by convention from the message type, optionally overridden on the Integration Command itself through the same `Wolverine Message` stereotype, since the queue belongs to the application that owns the Command.
...
The Golden Sample settles one path and one path only: RabbitMQ, Transactional Outbox None, Error Handling Policy Retry with cooldown, with the application declaring its own destinations.
...
<Table
  ...
  rows={[
    ...
    ["Always declaring the destinations the application uses", "R2.7", "**Golden Sample** — the declaring registration form is the sample's own path. The non-declaring alternative is withdrawn from this release"],
    ...
    ["Running alongside other Eventing Providers via the Composite Message Bus", "R9", "**EXTRAPOLATED.** Untouched by the sample, but with shipped precedent in the MassTransit and NServiceBus modules. Checked per R19.4 by installing Wolverine into the existing `CompositeMessageBus` Test Application"],
...
7. THE Software Factory SHALL generate code that declares the exchanges, topics and queues the application uses, for every application. THERE SHALL BE no Broker Topology setting and no non-declaring mode — see the callout below.
...
<Callout id="broker-topology-removed" tone="warning" title="R2.7 — Broker Topology is removed, and this costs a real capability">

An earlier revision exposed a Broker Topology setting with Auto-provision and Externally owned. It is withdrawn: the module has one behaviour, it always declares the destinations it uses, and there is no setting.

**What this costs, stated plainly.** An application whose broker credentials have no permission to declare a destination is no longer supported. It will attempt to declare on startup and the broker will refuse it. Joining a platform-team-owned estate is therefore out of scope for this release, and UJ-6 has been narrowed accordingly — Rashid can still match the platform team's exact names through the Topic Name and Destination Queue Name overrides, which is the half of that journey the module still delivers, but he cannot avoid the declaration attempt.

That is accepted rather than overlooked. Reinstating it is a setting plus one branch in the transport registration, so the door is not closed — but nothing in this release claims it works, and no criterion below may be read as promising it.
...
3. THE Wolverine eventing module SHALL provide an optional Topic Name override on an Integration Event, carried as a property of the single `Wolverine Message` stereotype of R9.8 and NOT as a stereotype of its own, and WHEN one is present THE Software Factory SHALL use it in place of the convention-derived name.
...
11. THE generated Message Bus contract implementation SHALL implement exactly the members the Message Bus contract declares in THAT application, and no others. IN PARTICULAR, an addressed send overload — one taking a destination address alongside the message — SHALL be emitted ONLY in an application whose contract declares it, since the contract gains that member only when another Eventing Provider that supports addressed sending is installed. AND WHERE it is emitted, THE implementation SHALL route the message to the address it was given; IF the address cannot be honoured, THE implementation SHALL fail rather than silently sending to the default destination.
...
3. THE Wolverine eventing module SHALL provide an optional Destination Queue Name override on the Integration Command element itself, carried as a property of the same single `Wolverine Message` stereotype of R9.8 and NOT as a stereotype of its own, since the queue an Integration Command is delivered to is declared by the application that owns the Command and must be identical for every application that sends it.
...
8. THE Wolverine eventing module SHALL define exactly one stereotype, named `Wolverine Message`, and SHALL NOT define `Message Topology Settings`, `Command Distribution`, or any other additional stereotype. THAT single stereotype SHALL be the one registered as the provider's applicable stereotype under R9.1, AND SHALL carry both the Topic Name override of R3.3 and the Destination Queue Name override of R4.3 as optional properties of its own.
9. `Wolverine Message` SHALL target both Integration Events and Integration Commands, AND each of its two override properties SHALL be hidden on the element type it does not apply to — Topic Name shown only on an Integration Event, Destination Queue Name only on an Integration Command — so that neither element type ever displays a property that is meaningless for it. SINCE R9.2 requires no designation at all when Wolverine is the only Eventing Provider installed, applying `Wolverine Message` by hand SHALL be necessary only in order to set an override; where two or more providers are installed the Message Bus stereotype applies it as the designation of R9.3.

<Callout id="single-stereotype-decision" tone="decision" title="R9.8 / R9.9 — one stereotype, not three, and why the property gating is a criterion">

The module previously defined three stereotypes: `Wolverine Message` as an empty designation marker, plus `Message Topology Settings` and `Command Distribution` cloned from MassTransit's names to carry the two overrides. That shape puts two boxes on a modeller's screen where one belongs, and it was never what was asked for.

Collapsing them is only tolerable because a stereotype property can be hidden conditionally: a Stereotype Definition Property carries an Is Active Function, a JavaScript predicate returning true or false that decides whether the property is displayed, and that predicate can read the element it is applied to. Shipped Intent modules already branch on exactly that — `return Ignore != "true" && element.specialization != "Service";` — so the mechanism is precedent, not invention.

R9.9 states the gating as a criterion rather than leaving it to the design, because without it a single stereotype is strictly worse than three: every Integration Event would show a Destination Queue Name box that does nothing, and every Integration Command a Topic Name box that does nothing. The consolidation is only a win if the gating comes with it.
...
R19.4 makes the existing `Tests/CompositeMessageBus` application the check — Wolverine is installed into it alongside the providers it already carries, rather than a bespoke multi-provider application being created. Until the resulting change to that application's committed output has been reviewed, treat R9 as designed rather than demonstrated.
...
2. THE Wolverine eventing module SHALL expose exactly three module settings — Transport, Transactional Outbox, Error Handling Policy — each with the options and default given in R2.1, R6.1 and R7.1. No setting SHALL be added for the Broker Topology, the Subscriber Queue naming rule, the serialization format, or the durable storage technology, each of which is either withdrawn or derived rather than chosen.
3. WHEN the Wolverine eventing module is installed and no setting is changed, THE Software Factory SHALL generate an application that compiles, configured for the Local Transport, Transactional Outbox None and Error Handling Policy Retry with cooldown, declaring its own destinations.
...
<Table
  ...
  rows={[
    ["`WolverineEventing.Publish.RabbitMQ` · `WolverineEventing.Subscribe.RabbitMQ`", "Transport RabbitMQ, Outbox None. The Golden Sample pair itself serves as this row's Test Application — a second `Wolverine.Transport.RabbitMQ` pair is NOT created, because it would model the identical configuration twice", "R2, R3, R4, R5"],
    ...
    ["`CompositeMessageBus` — the EXISTING shared Test Application", "Wolverine installed alongside the Eventing Providers that application already carries, with messages designated to each. A dedicated `Wolverine.MultiProvider` application is NOT created — see the callout below", "R9 — the check on an extrapolated requirement"],
    ["`Wolverine.MultiTenancy`", "Wolverine plus the Finbuckle multi-tenancy module", "R12 — the check on an extrapolated requirement"]
  ]}
/>

<Callout id="no-duplicate-test-apps" tone="decision" title="R19.1 / R19.4 — reuse an existing application rather than cloning its configuration">

Two rows of this matrix previously named applications that duplicated something already in `Tests/`, and both have been folded back.

**Multi-provider.** `Tests/CompositeMessageBus` already installs seven Eventing Providers and already has its generated output committed. A separate `Wolverine.MultiProvider` application reproduced that harness for one more provider while starting from zero committed files. Installing Wolverine into the existing application is strictly better evidence: R9's proof becomes a diff against output that was already under version control, which is exactly the regression signal R19 exists to produce, instead of a fresh tree where every file is new and nothing can be compared.

**RabbitMQ.** The Golden Sample pair and a `Wolverine.Transport.RabbitMQ` pair modelled the same Transport and the same Outbox setting.

The cost, accepted: `CompositeMessageBus` is not owned by this feature, so installing Wolverine into it will churn generated files that other providers' regression coverage also depends on. That churn must be reviewed as part of this feature rather than waved through, and a change there that is not attributable to adding Wolverine is a defect in this feature, not an unrelated one.

</Callout>

<Callout id="withdrawn-test-apps" tone="warning" title="R19.1 — three rows withdrawn, and the coverage they took with them">

Three applications named by an earlier revision of this matrix were never built, and rather than carry them as a standing gap they are withdrawn from the requirement. Each takes real coverage with it, recorded here so the loss is deliberate rather than discovered later.

**`Wolverine.Transport.Local`** — the Local transport, the module's default setting, now has no Test Application. A fresh install with nothing configured is the most common first experience of this module, and nothing in `Tests/` exercises it.

**`Wolverine.Topology.ExternallyOwned`** — the Topic Name and Destination Queue Name overrides now have no application-level coverage. R3.3 and R4.3 are verified at the template and stereotype level only; no committed generated output shows an override actually changing an emitted destination name.

**`Wolverine.Outbox.Durable.Postgresql`** — the durable outbox is covered on SQL Server only. R6.3's PostgreSQL branch (`PersistMessagesWithPostgresql`) is compile-verified against the package but never generated into a real application.
...
4. THE Test Application coverage for an extrapolated requirement — `CompositeMessageBus` carrying Wolverine for R9, and `Wolverine.MultiTenancy` for R12 — SHALL exist before that requirement is considered met, since no other evidence for either exists. FOR R9 specifically, THE Wolverine eventing module SHALL be installed into the existing `Tests/CompositeMessageBus` application rather than a new application being created, AND THE resulting change to that application's already-committed generated output SHALL itself be the evidence.
