...
namespace Ardalis.IntegrationTests.HttpClients.Clients
{
    public class ClientsHttpClient : IClientsService
    {
        ...
        public async Task<PagedResult<ClientDto>> GetClientsPaginatedAsync(
            int pageNo,
            int pageSize,
            CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/client/paginated";
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<PagedResult<ClientDto>>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
