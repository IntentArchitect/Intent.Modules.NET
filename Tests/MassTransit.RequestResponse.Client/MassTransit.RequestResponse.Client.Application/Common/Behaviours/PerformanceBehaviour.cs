        ...
        where TRequest : notnull
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (elapsedMilliseconds > 500)
            {
                ...
                {
                    _logger.LogWarning("MassTransit.RequestResponse.Client Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName} {@Request}", requestName, elapsedMilliseconds, user?.Id, user?.Name, request);
                }
                else
                {
                    _logger.LogWarning("MassTransit.RequestResponse.Client Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName}", requestName, elapsedMilliseconds, user?.Id, user?.Name);
                }
            }
            return response;
        }
