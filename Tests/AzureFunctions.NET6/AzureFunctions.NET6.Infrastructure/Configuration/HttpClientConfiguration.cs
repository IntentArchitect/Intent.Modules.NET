...
namespace AzureFunctions.NET6.Infrastructure.Configuration
{
    public static class HttpClientConfiguration
    {
        public static void AddHttpClients(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services
                ...
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "CustomersService");
                });

            services
                .AddHttpClient<IEnumService, EnumServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "EnumService");
                });

            services
                .AddHttpClient<IIgnoresService, IgnoresServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "IgnoresService");
                });

            services
                .AddHttpClient<IListedUnlistedServicesService, ListedUnlistedServicesServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "ListedUnlistedServicesService");
                });

            services
                .AddHttpClient<INullableResultService, NullableResultServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "NullableResultService");
                });

            services
                .AddHttpClient<IParamsService, ParamsServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "ParamsService");
                });

            services
                .AddHttpClient<ISampleDomainsService, SampleDomainsServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "SampleDomainsService");
                });

            services
                .AddHttpClient<IValidationService, ValidationServiceHttpClient>(http =>
                {
                    ApplyAppSettings(http, configuration, "AzureFunctions.NET6.Services", "ValidationService");
                });
        }
