...
namespace MongoDb.TestApplication.Infrastructure.Persistence.Mappings.Collections.FolderCollection
{
    public class FolderCollectionEntityAMapping : IMongoMappingConfiguration<FolderCollectionEntityA>
    {
        ...
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(FolderCollectionEntityA)))
            {
                BsonClassMap.RegisterClassMap<FolderCollectionEntityA>(
                    mapping =>
                    {
                        mapping.AutoMap();
