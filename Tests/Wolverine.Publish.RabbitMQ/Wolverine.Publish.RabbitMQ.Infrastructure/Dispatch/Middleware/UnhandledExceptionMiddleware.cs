...
namespace Wolverine.Publish.RabbitMQ.Infrastructure.Dispatch.Middleware
{
    public class UnhandledExceptionMiddleware
    {
        ...
        private void LogException(Exception exception, object? request, ILogger logger)
        {
            ...
            {
                logger.LogError(exception, "Wolverine.Publish.RabbitMQ Request: Unhandled Exception for Request {Name} {@Request}", requestName, request);
            }
            else
            {
                logger.LogError(exception, "Wolverine.Publish.RabbitMQ Request: Unhandled Exception for Request {Name}", requestName);
            }
        }
    }
}