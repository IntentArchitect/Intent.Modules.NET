    ...
    public class AISkillsStaticContentTemplateRegistration : AIStaticContentTemplateRegistration
    {
        ...
        public override IReadOnlyDictionary<string, string> Replacements(IOutputTarget outputTarget)
        {
            ...
            var automaticallyPersistUnitOfWork =
                settings.GetUnitOfWorkSettings()?.AutomaticallyPersistUnitOfWork() ?? true;
            ...
            var additionalCommandRules = new List<string>();
            var additionalQueryRules = new List<string>();

            if (automaticallyPersistUnitOfWork)
            {
                additionalCommandRules.Add("""
...
"""";
                ...
                additionalCommandRules.Add(autoMapperProjectionGuidance);
                additionalQueryRules.Add(autoMapperProjectionGuidance);
            }

            var result = new Dictionary<string, string>
            {
                ["Additional Command Rules"] = string.Join(Environment.NewLine + Environment.NewLine, additionalCommandRules),
                ["Additional Query Rules"] = string.Join(Environment.NewLine + Environment.NewLine, additionalQueryRules),
            };
