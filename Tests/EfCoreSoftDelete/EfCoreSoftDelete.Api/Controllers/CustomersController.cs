...
using EfCoreSoftDelete.Api.Controllers.ResponseTypes;
using EfCoreSoftDelete.Application.Customers;
using EfCoreSoftDelete.Application.Customers.CreateCustomer;
using EfCoreSoftDelete.Application.Customers.DeleteCustomer;
using EfCoreSoftDelete.Application.Customers.GetCustomerById;
using EfCoreSoftDelete.Application.Customers.GetCustomers;
using EfCoreSoftDelete.Application.Customers.GetDeletedCustomerById;
using EfCoreSoftDelete.Application.Customers.UpdateCustomer;
...
namespace EfCoreSoftDelete.Api.Controllers
{
    [ApiController]
    public class CustomersController : ControllerBase
    {
        ...
        /// <response code="400">One or more validation errors have occurred.</response>
        /// <response code="404">No CustomerDto could be found with the provided parameters.</response>
        [HttpGet("api/customers/deleted/{id}")]
        [ProducesResponseType(typeof(CustomerDto), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status500InternalServerError)]
        public async Task<ActionResult<CustomerDto>> GetDeletedCustomerById(
            [FromRoute] Guid id,
            CancellationToken cancellationToken = default)
        {
            var result = await _mediator.Send(new GetDeletedCustomerByIdQuery(id: id), cancellationToken);
            return result == null ? NotFound() : Ok(result);
        }
    }
}