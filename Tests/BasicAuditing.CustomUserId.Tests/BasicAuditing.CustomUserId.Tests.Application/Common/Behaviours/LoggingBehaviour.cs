...
namespace BasicAuditing.CustomUserId.Tests.Application.Common.Behaviours
{
        ...
        where TRequest : notnull
    {
        ...
        public async Task Process(TRequest request, CancellationToken cancellationToken)
        {
            ...
            {
                _logger.LogInformation("BasicAuditing.CustomUserId.Tests Request: {Name} {@UserId} {@UserName} {@Request}", requestName, user?.Id, user?.Name, request);
            }
            else
            {
                _logger.LogInformation("BasicAuditing.CustomUserId.Tests Request: {Name} {@UserId} {@UserName}", requestName, user?.Id, user?.Name);
            }
        }
    }
}