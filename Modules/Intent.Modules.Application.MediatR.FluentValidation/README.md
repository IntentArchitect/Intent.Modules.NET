...
By default, a `validator` is automatically generated for every `Command` and `Query`. This includes cases where the validator would otherwise be empty (unlike the validators for `DTOs`, which are only created when necessary: [see here](https://github.com/IntentArchitect/Intent.Modules.NET/blob/master/Modules/Intent.Modules.Application.FluentValidation.Dtos/README.md#creating-dto-validators)).
...
![Stub Validators Setting](resources/images/stub-validators.png)
