...
**2 modules created**, **1 changed**, **1 read but untouched**. Inside the new eventing module: **4 C# templates**, **4 factory extensions**, **1 module settings configuration with 3 fields**, **1 message bus provider**, **1 stereotype definition**, **7 NuGet package registrations**.
...
The `Destination Queue Name` property is defined on the `Wolverine Message` stereotype and displayed only when that stereotype sits on an `Integration Command`, per R9.9. MassTransit puts its equivalent on the `Send Integration Command Target End` association.
...
<ModelChanges id="eventing-templates" caption="Intent.Eventing.Wolverine — templates and factory extensions" changes={[
  ...
  { element: "WolverineMessageBus", designer: "Module Builder", change: "added", note: "C# Template, C# File Builder, Single File. Role `Infrastructure.Eventing.WolverineEventBus`, Default Location `Eventing`. The buffered Message Bus contract implementation with its flush method. Emits an addressed-send overload ONLY where the application's contract declares one — which happens only when MassTransit is installed — and honours the address when it does. Satisfies R3.8, R3.11, R4.1, R8.8." },
...
<DataModel
  ...
  caption="R14.2 — exactly three fields, each a Select control with Is Required true"
  fields={[
    ...
    { name: "Broker Topology", type: "Select — Auto-provision, Externally owned", change: "removed", note: "WITHDRAWN per R2.7. The field is not created; if it already exists in the model it is deleted. The module always declares the destinations it uses" }
  ]}
/>
...
No fourth setting is added. R14.2 forbids one for the Broker Topology, the Subscriber Queue naming rule, the serialization format and the durable storage technology — the first withdrawn, the rest derived.

### Stereotype definitions

<Table
  id="stereotypes"
  caption="R9.8 — ONE stereotype definition, created under a `Stereotypes` folder in the module package"
  columns={["Stereotype", "Applicable to", "Properties", "Realizes"]}
  rows={[
    ["`Wolverine Message`", "`Message`, `Integration Command`", "`Topic Name` — text box, optional, Is Active Function `return element.specialization === 'Message';` · `Destination Queue Name` — text box, optional, Is Active Function `return element.specialization === 'Integration Command';`", "R9.1, R9.8, R9.9, R3.3 to R3.6, R4.3 to R4.6. Referenced by `Message Bus Provider Settings.Applicable Stereotypes`, which is how the provider becomes selectable on the Message Bus stereotype; both overrides are validated for emptiness and a 250-character ceiling by the template"]
  ]}
/>

R9.8 requires this to be the module's **only** stereotype. `Message Topology Settings` and `Command Distribution` are not created — the two override properties they used to carry move onto `Wolverine Message` itself, each gated by an Is Active Function so it is displayed only on the element type it means something for. That gating is what makes one stereotype an improvement rather than a regression, and R9.9 states it as a criterion for exactly that reason.

<Callout id="is-active-precedent" tone="info" title="The Is Active Function can read the element it is applied to">

A Stereotype Definition Property carries an `Is Active Function` — a JavaScript predicate returning true or false that decides whether the property is displayed. Its scope includes the sibling properties by name and an `element` object, so branching on the element's type is supported rather than assumed: shipped Intent modules in this repository's own module cache already do it, for example `return Ignore != "true" && element.specialization != "Service";` and `return EndpointType == "Receive Endpoint";`.
...
<Table
  id="test-apps"
  density="compact"
  caption="R19 — Test Applications with committed generated output, mirroring the MassTransit and NServiceBus coverage in `Tests/`. Three rows (Transport.Local, Topology.ExternallyOwned, Outbox.Postgresql) were withdrawn — see the requirements' withdrawn-test-apps callout"
  columns={["Test Application", "Configuration", "Requirements"]}
  rows={[
    ["`WolverineEventing.Publish.RabbitMQ` · `WolverineEventing.Subscribe.RabbitMQ`", "RabbitMQ, Outbox None — the Golden Sample pair itself, serving as this row's Test Application. No second `Wolverine.Transport.RabbitMQ` pair is created", "R2, R3, R4, R5"],
    ...
    ["`CompositeMessageBus` — existing, extended", "Wolverine installed into the existing shared Test Application alongside the seven Eventing Providers it already carries. No `Wolverine.MultiProvider` application is created", "R9 — sole evidence"],
...
<Table
  ...
  rows={[
    ...
    ["R2", "4-option Transport field; 4 transport NuGet packages. NO Broker Topology field — withdrawn per R2.7", "Transport branch in `WolverineEventingConfiguration`, always declaring destinations; appsettings registration requests in `WolverineEventingRegistrationExtension`; R2.6 fail-fast read", "R18, D6 probe for R2.6"],
    ["R3", "`Topic Name` property on the single `Wolverine Message` stereotype; `WolverineMessageBus` template element", "Publish-rule emission; kebab-case convention; override validation; the property's Is Active Function; buffered bus with flush; conditional DI registration", "R1, R18"],
    ["R4", "`Destination Queue Name` property on the same `Wolverine Message` stereotype, targeting Integration Command", "Send-rule emission and override resolution from the single declaration site; the property's Is Active Function", "R1, R3"],
    ...
    ["R14", "Module Settings Configuration with 3 fields; module dependency on Intent.Eventing.Contracts", "Uninstall behaviour; documentation of additive-only appsettings", "R2, R6, R7"],
...
The Domain → Services → UI ordering that governs application specs does not apply here: this feature mutates only the Module Builder designer. Where it reads other designers it reads the Services designer's **existing** message and subscription elements, adding nothing to them beyond the single `Wolverine Message` stereotype definition, which is declared in the Wolverine module's own package and attaches by `applicableTo`.
...
<Checklist
  ...
  items={[
    ...
    { id: "d-a2", label: "R9's designation filtering rests on routing, not on registration", note: "With conventional discovery on, a handler for a message designated to MassTransit is also registered with Wolverine. It receives nothing, because Wolverine has no listener bound for that message and the Composite Message Bus routes its publishes elsewhere. The existing `CompositeMessageBus` application, with Wolverine installed alongside the providers it already carries, is the Test Application that has to demonstrate this holds." },
    ...
    { id: "d-a6", label: "The module defines exactly ONE stereotype, and its two override properties are conditionally displayed", note: "Superseded the earlier assumption that stereotype names should mirror MassTransit's. `Message Topology Settings` and `Command Distribution` are NOT created; `Topic Name` and `Destination Queue Name` become properties of `Wolverine Message`, each gated by an Is Active Function on `element.specialization` so neither element type shows a property that is meaningless for it. R9.8 and R9.9 make this binding. The familiar MassTransit property NAMES are preserved, so a migrator still meets the same vocabulary — it is the extra stereotypes that are dropped, not the terminology." },
