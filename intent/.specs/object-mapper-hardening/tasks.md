...
- [ ] T1. Golden Sample — `ObjectMapping.Strict`
  - [ ] T1.1 [module] (satisfies: R8.1, R8.3, R8.11) — Create the `ObjectMapping.Strict` application and install its module set
  ...
  - [ ] T1.2 [model] (satisfies: R6.1, R8.2) — Model the Domain package (8 entities, 2 nullable hops)
  ...
  - [ ] T1.3 [model] (satisfies: R6.1, R2.5, R8.2, R8.9) — Model the Services package: DTOs pinning all 21 Mapping Shapes, plus the queries
    ...
    - Queries modelled via Domain Interactions: `GetOrderById` (single), `GetOrders` (collection), `GetOrderOrNull` (nullable single), `GetOrdersPaged` (paged), `GetOrdersCursorPaged` (cursor-paged) — R8.9 needs the last two present end to end
    - Leave `OrderDto` with at least one unmapped field to pin R6.4 (omitted from the initializer, no warning)
  - [ ] T1.4 [code] (satisfies: R1.2, R6.1, R9.1, R9.2, R9.4) — Hand-write `OrderDtoMappingExtensions` exactly as design A.3
  ...
  - [ ] T1.5 [code] (satisfies: R6.1, R6.5, R6.7) — Hand-write the remaining 5 Mapping Extension Classes
  ...
  - [ ] T1.6 [code] (satisfies: R3.2, R3.3, R3.4, R3.5, R4.1, R4.3) — Hand-write the Golden Sample Call Sites in the five query handlers
    - `return order.MapToOrderDto();` · `return orders.MapToOrderDtoList();` · `return order?.MapToOrderDto();` · `return orders.MapToPagedResult(x => x.MapToOrderDto());` · `return orders.MapToCursorPagedResult(x => x.MapToOrderDto());`
  ...
  - [ ] T1.7 [code] (satisfies: R6.1, R8.4, R1.6) — Write the Mapping Shape test suite
  ...
  - [ ] T1.8 [code] (satisfies: R8.8, R8.9, R4.2, R4.4) — Write the Call Site and pagination test suite
    ...
    - R8.9/R4.2/R4.4: paged and cursor-paged queries end to end — populated page with total count / page number / page size intact, and an empty page that does not throw and keeps its metadata
  - [ ] T1.9 [code] (satisfies: R8.6) — Snapshot the green Strict baseline into the spec folder
...
- [ ] T2. Golden Sample — `ObjectMapping.Lenient`
  - [ ] T2.1 [module] (satisfies: R8.1, R8.3, R8.11) — Create the `ObjectMapping.Lenient` application and install its module set
  ...
  - [ ] T2.2 [model] (satisfies: R6.1, R8.2) — Model the Domain package, element-for-element identical to T1.2
  ...
  - [ ] T2.3 [model] (satisfies: R6.1, R2.5, R8.2, R8.9) — Model the Services package, element-for-element identical to T1.3
    - Same DTOs, same field order, same two Prefix Form expressions, same five queries including paged and cursor-paged
  - [ ] T2.4 [code] (satisfies: R1.3, R6.1, R9.1, R9.2, R9.8) — Hand-write `OrderDtoMappingExtensions` in the Lenient form (design A.4)
  ...
  - [ ] T2.5 [code] (satisfies: R6.1, R6.5, R6.7) — Hand-write the remaining 5 Mapping Extension Classes
  ...
  - [ ] T2.6 [code] (satisfies: R3.2, R3.3, R3.4, R3.5, R4.1, R4.3) — Hand-write the Golden Sample Call Sites in the five query handlers
    - Identical statements to T1.6 — the null-handling setting does not reach Call Sites, only mapping expressions
  - [ ] T2.7 [code] (satisfies: R6.1, R8.4, R1.7) — Write the Mapping Shape test suite
  ...
  - [ ] T2.8 [code] (satisfies: R8.8, R8.9, R4.2, R4.4) — Write the Call Site and pagination test suite
    - Mirrors T1.8: reflection assertion that no `IMapper` is injected, behavioural assertion the DTO is populated, and populated/empty paged and cursor-paged journeys
  - [ ] T2.9 [code] (satisfies: R8.6) — Snapshot the green Lenient baseline into the spec folder
    - Same as T1.9, into `intent/.specs/object-mapper-hardening/baseline/lenient/`

- [ ] T3. Module metamodel — the `Null Path Handling` setting
  - [ ] T3.1 [model] (satisfies: R1.1) — Add the `Object Mapping` Settings Group and the `Null Path Handling` select setting
  ...
  - [ ] T3.2 [code] (satisfies: R3.1, R8.12) — Add the missing `interoperability` detect and bump the module version
...
- [ ] T4. Object Mapper expression builder
  - [ ] T4.1 [code] (satisfies: R1.2, R1.3, R1.4, R1.5, R1.8, R1.9, R9.8) — Per-hop null handling in `MappingHelper.BuildPath`
  ...
  - [ ] T4.2 [code] (satisfies: R2.1, R2.2, R2.3, R2.4, R2.5, R2.6, R2.7, R2.8) — Prefix normalisation in `BuildEntryExpression`'s expression branch
  ...
  - [ ] T4.3 [code] (satisfies: R6.2, R6.3, R6.4, R6.6, R6.7, R6.8) — Collection nullability across the three collection call sites
  ...
  - [ ] T4.4 [code] (satisfies: R7.1, R7.2, R7.3, R7.4) — Diagnostics and namespace derivation
  ...
  - [ ] T4.5 [code] (satisfies: R5.1, R5.2, R5.3) — Delete the AutoMapper stand-down guard from `CanRunTemplate`
  ...
  - [ ] T4.6 [code] (satisfies: R9.1, R9.2, R9.3, R9.4, R9.5, R9.6, R9.7) — Conform the template's output shape and managed mode
...
- [ ] T5. Domain Interactions call sites
  - [ ] T5.1 [code] (satisfies: R3.1, R3.2, R3.3, R3.4, R3.5, R3.6, R3.7, R3.8) — Add `ObjectMappingMappingStrategy`
  ...
  - [ ] T5.2 [code] (satisfies: R4.1, R4.2, R4.3, R4.4, R4.5) — Implement `ImplementPagedMappingStatement` on the same strategy
    - `return {entity}.{mappingMethod}(x => x.MapTo{Dto}());` — the projection is the strategy's only contribution; page and cursor metadata come from the pagination module untouched (R4.2, R4.3)
    - Never leave a paged return unmapped or defaulted (R4.5); an empty match returns an empty page with metadata intact (R4.4)
  - [ ] T5.3 [code] (satisfies: R3.1, R8.12) — Register the strategy and bump `Intent.Application.DomainInteractions`
...
- [ ] T6. Dogfood & Software Factory parity
  - [ ] T6.1 [module] (satisfies: R5.1, R8.6) — Install the rebuilt modules into both Flavour Applications
  ...
  - [ ] T6.2 [code] (satisfies: R8.5, R8.6, R8.7, R9.6, R1.9) — Delete the hand-written Golden Sample, regenerate, and diff against the baseline
  ...
  - [ ] T6.3 [code] (satisfies: R8.10) — Write the flavour-equivalence script
  ...
  - [ ] T6.4 [code] (satisfies: R8.12) — Update the module's README and release notes
