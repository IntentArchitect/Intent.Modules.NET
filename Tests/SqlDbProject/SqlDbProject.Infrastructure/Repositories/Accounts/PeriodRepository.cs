...
namespace SqlDbProject.Infrastructure.Repositories.Accounts
{
    ...
    public class PeriodRepository : RepositoryBase<Period, Period, ApplicationDbContext>, IPeriodRepository
    {
        ...
        public async Task<Period?> FindByIdAsync(
            int periodId,
            Func<IQueryable<Period>, IQueryable<Period>> queryOptions,
            CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.PeriodId == periodId, queryOptions, cancellationToken);
        }
