...
namespace EfCoreSoftDelete.Infrastructure.Repositories
{
    ...
    public class CustomerRepository : RepositoryBase<Customer, Customer, ApplicationDbContext>, ICustomerRepository
    {
        ...
        public async Task<Customer?> FindDeletedByIdAsync(Guid id, CancellationToken cancellationToken = default)
        {
            return await GetSet().IgnoreQueryFilters().FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
        }
