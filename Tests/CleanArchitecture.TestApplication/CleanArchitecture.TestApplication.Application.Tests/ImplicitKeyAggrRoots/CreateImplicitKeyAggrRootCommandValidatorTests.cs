...
namespace CleanArchitecture.TestApplication.Application.Tests.ImplicitKeyAggrRoots
{
    public class CreateImplicitKeyAggrRootCommandValidatorTests
    {
        ...
        public async Task Validate_WithInvalidCommand_FailsValidation(CreateImplicitKeyAggrRootCommand testCommand, string expectedPropertyName, string expectedPhrase)
        {
            // Arrange
            var validator = GetValidationBehaviour();
            var expectedId = new Fixture().Create<System.Guid>();
        ...
        {
            return new ValidationBehaviour<CreateImplicitKeyAggrRootCommand, System.Guid>(new[] { new CreateImplicitKeyAggrRootCommandValidator() });
        }
    }
}