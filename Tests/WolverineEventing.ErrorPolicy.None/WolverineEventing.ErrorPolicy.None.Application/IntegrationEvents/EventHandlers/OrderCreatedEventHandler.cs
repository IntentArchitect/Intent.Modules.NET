...
namespace WolverineEventing.ErrorPolicy.None.Application.IntegrationEvents.EventHandlers
{
    ...
    public class OrderCreatedEventHandler : IIntegrationEventHandler<OrderCreatedEvent>
    {
        ...
        {
            // Throws on every delivery, on purpose. This is what makes Error Handling Policy = None
            // observable: no handler ever succeeds, so every delivery goes straight to the error
            // queue with no retry. Logged before throwing so each delivery attempt is timestamped.
