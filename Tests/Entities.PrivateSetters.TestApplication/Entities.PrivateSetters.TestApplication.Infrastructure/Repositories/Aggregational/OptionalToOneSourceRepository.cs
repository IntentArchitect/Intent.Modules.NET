...
namespace Entities.PrivateSetters.TestApplication.Infrastructure.Repositories.Aggregational
{
    ...
    public class OptionalToOneSourceRepository : RepositoryBase<OptionalToOneSource, OptionalToOneSource, ApplicationDbContext>, IOptionalToOneSourceRepository
    {
        public OptionalToOneSourceRepository(ApplicationDbContext dbContext, IMapper mapper) : base(dbContext, mapper)
        {
        }

        public async Task<OptionalToOneSource?> FindByIdAsync(Guid id, CancellationToken cancellationToken = default)
        {
