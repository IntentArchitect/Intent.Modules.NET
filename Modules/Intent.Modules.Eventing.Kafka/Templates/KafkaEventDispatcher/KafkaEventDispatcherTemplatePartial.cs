...
namespace Intent.Modules.Eventing.Kafka.Templates.KafkaEventDispatcher
{
    [IntentManaged(Mode.Fully, Body = Mode.Merge)]
    public partial class KafkaEventDispatcherTemplate : CSharpTemplateBase<object>, ICSharpFileBuilderTemplate
    {
        public const string TemplateId = "Intent.Eventing.Kafka.KafkaEventDispatcher";

        [IntentManaged(Mode.Fully, Body = Mode.Ignore)]
        public KafkaEventDispatcherTemplate(IOutputTarget outputTarget, object model = null) : base(TemplateId, outputTarget, model)
        {
            CSharpFile = new CSharpFile(this.GetNamespace(), this.GetFolderPath())
                ...
                .AddClass($"KafkaEventDispatcher", @class =>
                {
                    @class.AddGenericParameter("T", out var t);
                    @class.AddGenericTypeConstraint(t, c => c.AddType("class"));
                    @class.ImplementsInterface($"{this.GetKafkaEventDispatcherInterfaceName()}<{t}>");
