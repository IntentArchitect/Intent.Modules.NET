...
namespace AdvancedMappingCrud.Cosmos.Tests.Infrastructure.Repositories.ManyToMany
{
    internal class CategoryCosmosDBRepository : CosmosDBRepositoryBase<Category, CategoryDocument, ICategoryDocument>, ICategoryRepository
    {
        public CategoryCosmosDBRepository(CosmosDBUnitOfWork unitOfWork,
            IRepository<CategoryDocument> cosmosRepository,
        ...
        public async Task<Category?> FindByIdAsync(Guid id, CancellationToken cancellationToken = default) => await FindByIdAsync(id: id.ToString(), cancellationToken: cancellationToken);

        public async Task<List<Category>> FindByIdsAsync(Guid[] ids, CancellationToken cancellationToken = default) => await FindByIdsAsync(ids.Select(id => id.ToString()).ToArray(), cancellationToken);
    }
}