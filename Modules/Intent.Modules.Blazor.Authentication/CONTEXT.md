...
## Invariant: `AccessTokenResult(RequiresRedirect, ...)` needs a non-null `InteractiveRequestOptions` (2026-09-11)

`AccessTokenNotAvailableException.Redirect()` (decompiled from the shipped `Microsoft.AspNetCore.Components.WebAssembly.Authentication` assembly, 8.0.11 through 10.0.9 — the constructor and `Redirect()` behaviour is unchanged across those targets) is: `if (_tokenResult.InteractionOptions != null && _tokenResult.InteractiveRequestUrl != null) _navigation.NavigateToLogin(...); else _navigation.NavigateTo(_tokenResult.RedirectUrl);`.

The working branch needs **both** `InteractiveRequestUrl` _and_ `InteractionOptions` non-null. The 4-arg `AccessTokenResult` constructor `(status, token, interactiveRequestUrl, interactiveRequest)` sets `InteractiveRequestUrl` correctly from its 3rd argument, but if the 4th argument (`interactiveRequest`) is passed as `null`, `Redirect()` falls into the `else` branch and reads `RedirectUrl` instead — a property only the separate, obsolete 3-arg constructor `(status, token, redirectUrl)` ever sets. Since the 4-arg constructor never touches it, `RedirectUrl` stays at its default (`null`), and `Redirect()` navigates to `null` / no-ops instead of the login page.

**The fix is not to reach for the obsolete 3-arg constructor** — it is to actually populate the 4th argument: `new AccessTokenResult(AccessTokenResultStatus.RequiresRedirect, null, loginUrl, new InteractiveRequestOptions { Interaction = InteractionType.GetToken, ReturnUrl = returnUrl })`. This is the officially-supported, non-obsolete path — it's exactly what the SDK's own `NavigationManagerExtensions.NavigateToLogin(manager, loginPath)` convenience overload does internally when building its own request. `PersistentAuthenticationStateProviderTemplatePartial.cs` does this instead of suppressing CS0618 around the obsolete overload.

## Non-finding: `IAccessTokenProvider` DI override is not needed (2026-09-11)

An external bug report claimed `ClientAddAuthentication` must explicitly re-register `AuthenticationStateProvider` and `IAccessTokenProvider` **after** `AddApiAuthorization()`, arguing registration order determines which provider `IAccessTokenProvider` resolves to. Investigated and not reproducible against the actual framework behaviour:

`AddApiAuthorization()` registers both `AuthenticationStateProvider` and `IAccessTokenProvider` via `TryAddScoped` (confirmed by decompiling the shipped SDK, net6 through net10). Its `IAccessTokenProvider` registration is a **lazy factory** — `sp =>
(IAccessTokenProvider)sp.GetRequiredService<AuthenticationStateProvider>()` — resolved at call time, not at registration time. `ClientAddAuthentication` registers the custom `PersistentAuthenticationStateProvider` as `AuthenticationStateProvider` via a plain `AddSingleton` (not `TryAdd`), which always wins as the final descriptor **regardless of call order** relative to `AddApiAuthorization()`. So `IAccessTokenProvider` already resolves to the custom provider today, with no explicit override needed.

**Do not add an explicit `IAccessTokenProvider` override on the strength of an external report alone — re-verify against the then-current SDK first.** This non-finding depends on `AddApiAuthorization()`'s use of `TryAddScoped`/a lazy factory, which is SDK behaviour this module does not control and which could change in a future release.
