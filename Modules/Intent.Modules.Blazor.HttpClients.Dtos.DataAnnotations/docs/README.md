...
For each property, it will add all the appropriate attributes that it can from the [System.ComponentModel.DataAnnotations](https://learn.microsoft.com/dotnet/api/system.componentmodel.dataannotations) namespace based on any [Validation Rules](xref:modules-dotnet.intent-application-fluentvalidation) applied to the field in the Services designer as well as any [implicit/inferred validation rules](xref:modules-dotnet.intent-application-fluentvalidation#implicitinferred-validation-rules) which may be applicable.
