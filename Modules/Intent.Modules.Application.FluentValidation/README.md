...
While the default available rules will be able to handle most validation use cases, if a custom validation rule is required, the `Custom` or `Must` option can be selected. This will create a method in the validator, ignored by Intent Architect, which allows for custom validation rules.
...
For details on all the different rules and how they work, refer to the [FluentValidation Docs](https://docs.fluentvalidation.net/en/latest/built-in-validators.html).
