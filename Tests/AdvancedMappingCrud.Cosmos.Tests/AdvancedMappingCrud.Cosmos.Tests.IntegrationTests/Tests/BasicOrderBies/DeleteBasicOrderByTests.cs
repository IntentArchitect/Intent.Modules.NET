...
namespace AdvancedMappingCrud.Cosmos.Tests.IntegrationTests.Tests.BasicOrderBies
{
    ...
    public class DeleteBasicOrderByTests : BaseIntegrationTest
    {
        public DeleteBasicOrderByTests(IntegrationTestWebAppFactory factory) : base(factory)
        {
        }
        ...
        public async Task DeleteBasicOrderBy_ShouldDeleteBasicOrderBy()
        {
            ...
            // Act
            await client.DeleteBasicOrderByAsync(basicOrderById, TestContext.Current.CancellationToken);

            // Assert
            var exception = await Assert.ThrowsAsync<HttpClientRequestException>(() => client.GetBasicOrderByByIdAsync(basicOrderById, TestContext.Current.CancellationToken));
