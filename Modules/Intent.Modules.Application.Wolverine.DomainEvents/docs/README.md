...
This module generates an AI agent skill file at `.agents/skills/wolverine-domain-event-handler/SKILL.md`, describing how to implement or revise Wolverine domain event handlers while preserving Clean Architecture layer boundaries and existing codebase conventions. It documents Wolverine's convention-based `Handle` method dispatch (as opposed to MediatR's `INotificationHandler<T>` interface), idempotency expectations for at-least-once delivery, and side-effect guidance for integration events, notifications, projections, and auditing. It mirrors the equivalent skill file generated by `Intent.MediatR.DomainEvents`.
