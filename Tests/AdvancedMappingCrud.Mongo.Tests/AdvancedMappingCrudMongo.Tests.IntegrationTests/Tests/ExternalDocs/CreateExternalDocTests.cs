...
namespace AdvancedMappingCrudMongo.Tests.IntegrationTests.Tests.ExternalDocs
{
    ...
    public class CreateExternalDocTests : BaseIntegrationTest
    {
        ...
        public async Task CreateExternalDoc_ShouldCreateExternalDoc()
        {
            ...
            // Act
            var externalDocId = await client.CreateExternalDocAsync(command, TestContext.Current.CancellationToken);

            // Assert
            var externalDoc = await client.GetExternalDocByIdAsync(externalDocId, TestContext.Current.CancellationToken);
