...
namespace AdvancedMapping.Repositories.Mapperly.Tests.Infrastructure.Repositories.Sales
{
    [IntentManaged(Mode.Merge, Signature = Mode.Fully)]
    public class ProductCategoryRepository : RepositoryBase<ProductCategory, ProductCategory, ApplicationDbContext>, IProductCategoryRepository
    {
        ...
        public async Task<ProductCategory?> FindByIdAsync(
            Guid id,
            Func<IQueryable<ProductCategory>, IQueryable<ProductCategory>> queryOptions,
            CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.Id == id, queryOptions, cancellationToken);
        }

        public async Task<List<ProductCategory>> FindByIdsAsync(Guid[] ids, CancellationToken cancellationToken = default)
        {
