...
namespace Intent.Modules.Aws.DynamoDB.FactoryExtensions
{
    ...
    {
        public override string Id => "Intent.Aws.DynamoDB.EnsureParameterlessConstructorsExtension";
        ...
        protected override void OnAfterTemplateRegistrations(IApplication application)
        {
            ...
            foreach (var model in classModels)
            {
                ...
                entityTemplate?.CSharpFile.OnBuild(file =>
                {
                    ...
                    var entityImplConstructors = file.Classes.First().Constructors;
                    var stateConstructors = stateTemplate?.CSharpFile.Classes.First().Constructors ?? [];
                    var collectiveConstructors = entityImplConstructors.Concat(stateConstructors).ToArray();

                    if (collectiveConstructors.Length == 0 || collectiveConstructors.Any(x => !x.Parameters.Any()))
                    {
                        return;
                    }
