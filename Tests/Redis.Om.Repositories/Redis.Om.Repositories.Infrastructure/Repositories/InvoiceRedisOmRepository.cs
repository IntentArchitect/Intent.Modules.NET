...
{
    internal class InvoiceRedisOmRepository : RedisOmRepositoryBase<Invoice, InvoiceDocument, IInvoiceDocument>, IInvoiceRepository
    {
        public InvoiceRedisOmRepository(RedisOmUnitOfWork unitOfWork, RedisConnectionProvider connectionProvider) : base(unitOfWork, connectionProvider)
        {
        }
        ...
        protected override string GetIdValue(Invoice entity) => entity.Id;

        protected override void SetIdValue(Invoice domainEntity, InvoiceDocument document) => ReflectionHelper.ForceSetProperty(domainEntity, "Id", document.Id);
    }
}