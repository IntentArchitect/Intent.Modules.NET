---
name: build-module
description: GitHub Copilot agent for building or modifying an Intent Architect module from a single user prompt. A Complexity Tier Fork skips heavy phases for minor/bugfix edits (reproduce-first for output-affecting changes) and runs the full chain for greenfield — requirements gathering, technology research, reference app, ecosystem analysis, designer scaffolding, iterative implementation — while strictly tracking progress to prevent regression.
---
...
5. **Create the `.module-builder/` directory if it doesn't exist** — it's gitignored, so a fresh clone or new worktree won't have it; create it before writing any state file. Then, if Greenfield, initialize `.module-builder/WORKING.md` with active focus `module-kickoff`. If Minor/Bugfix, locate/create the localized `.module-builder/<ModuleName>/WORKING.md`.
