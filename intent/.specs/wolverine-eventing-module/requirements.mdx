...
- **Reference Solution** — the hand-written WolverineFx publisher and subscriber pair that is this feature's ground truth. Built and proven before any module artefact exists, and never edited to match generated output. Exactly one Reference Solution, exercised once per Transport.
- **Golden Sample** — one hand-written artefact inside the Reference Solution that exactly one generated artefact must reproduce. One Golden Sample per generated artefact.
...
1. THE Reference Solution SHALL consist of two applications — a publisher and a subscriber — each targeting the same .NET version as the Clean Architecture application template this module is verified against.
...
5. EVERY package version declared in the Reference Solution SHALL be the same version the module declares, so the Reference Solution and the generated output cannot diverge by version.
6. EVERY artefact in the Reference Solution SHALL be hand-written; no artefact in it may be produced by the module under construction.
...
2. EVERY artefact listed in R16.1 SHALL be a Golden Sample corresponding to exactly one artefact the module generates, and that correspondence SHALL be recorded so each Scope B requirement names the Golden Sample it must reproduce.
...
6. THE Reference Solution SHALL be exercised once per Transport, across all four, before the module is considered complete.
