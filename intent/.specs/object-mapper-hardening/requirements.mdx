...
- **UJ-3. Priya's existing free-form expression mappings survive the swap.**
  ...
  - **Climax:** Every `src.`-prefixed Expression Mapping generates against this module's source parameter and evaluates to exactly what AutoMapper evaluated to. She knows it worked because she changed zero mappings in the designer and no value moved.
  - **Resolution:** No designer rework. An expression left in the `src.` form can still be swapped back to AutoMapper untouched; one rewritten to `projectFrom.` cannot, because AutoMapper does not recognise that prefix.
  - **Edge case:** One expression was authored bare, with no prefix at all. It generates the same unresolved identifier AutoMapper generated for it — the swap neither fixes nor worsens it — so she rewrites that one with a prefix.

```mermaid
flowchart TD
  A[Expression Mapping in the Services model] --> B{Prefix Form as authored}
  B -->|src dot - R2.2| C[Prefix rewritten to the source parameter]
  B -->|projectFrom dot - R2.3| C
  C --> D[Both recognised forms produce byte-identical output - R2.5]
  D --> E[Migrated model needs no rework and no value moves - R2.1]
  B -->|neither form - R2.4| F[Source parameter prepended to the whole expression]
  F --> G[Edge - same unresolved identifier AutoMapper produced, no new breakage - R2.8]
  G --> H[Author rewrites that one with a prefix]
  H --> B
```
...
- **Prefix Form** — how an Expression Mapping names the source object. Exactly two are **recognised**: `src.` (AutoMapper's convention) and `projectFrom.` (this module's own). An expression in neither form is *unprefixed*, and is handled exactly as AutoMapper handles it — the source parameter is prepended to the expression as a whole, which is correct only when the expression is a single member reference.
...
- **Fixing unprefixed expression mappings.** An Expression Mapping in neither recognised Prefix Form is handled exactly as `Intent.Application.Dtos.AutoMapper` handles it — including the cases AutoMapper gets wrong, such as a multi-identifier expression or one referencing no source member at all. Resolving those identifiers against the mapped entity would make this module better than both incumbents, and is deliberately not done: a drop-in replacement must generate the same thing, not the better thing.
- **Collection types other than `List`.** Marking a DTO field as a collection produces `List<T>` and nothing else — the Dtos module sets that formatter unconditionally — so array, `ICollection` and `IEnumerable` targets are out of scope rather than unimplemented.
...
### Requirement 2: Expression mappings behave exactly as AutoMapper's do

**User Story:** As a developer migrating off AutoMapper, I want my existing free-form expression mappings to generate exactly what AutoMapper generated for them, so that swapping the Mapping Provider changes the value of no mapping I already have.
...
<Table
  id="r2-prefixes"
  density="compact"
  caption="R2.1–R2.4 — how each authored form is handled, and how that compares to AutoMapper"
  columns={["Expression as authored", "AutoMapper generates", "This module generates", "Outcome"]}
  rows={[
    ["`src.Amount > 0 ? src.Amount : 0`", "Prefix recognised, left as its source parameter", "Prefix rewritten to this module's source parameter", "Correct in both"],
    ["`projectFrom.Amount > 0 ? projectFrom.Amount : 0`", "Prefix not recognised, its own prefix prepended — broken", "Prefix recognised, left as the source parameter", "Correct here only"],
    ["`Amount > 0 ? Amount : 0` — bare", "Its source parameter prepended to the whole expression, leaving the second identifier unresolved", "Same, with this module's source parameter", "Broken in both, by parity"],
    ["`DateTime.UtcNow` — no source member at all", "Its source parameter prepended — broken", "Same, with this module's source parameter", "Broken in both, by parity"]
  ]}
/>

#### Acceptance Criteria

1. THE Object Mapper module SHALL apply the same prefixing algorithm as `Intent.Application.Dtos.AutoMapper`: an Expression Mapping whose leading text is a recognised Prefix Form SHALL have that prefix rewritten to this module's source parameter, and any other Expression Mapping SHALL have this module's source parameter prepended to the expression as a whole.
2. WHEN an Expression Mapping is authored in the `src.` Prefix Form, THE Object Mapper module SHALL rewrite that prefix to this module's source parameter and SHALL NOT prepend a second prefix.
3. WHEN an Expression Mapping is authored in the `projectFrom.` Prefix Form, THE Object Mapper module SHALL treat it as already prefixed and SHALL NOT prepend a second prefix.
4. WHEN an Expression Mapping is in neither recognised Prefix Form, THE Object Mapper module SHALL prepend this module's source parameter to the expression as a whole, exactly as AutoMapper does, and SHALL NOT attempt to qualify individual member accesses within it.
5. THE Object Mapper module SHALL generate byte-identical output for two Expression Mappings that are semantically identical and differ only in being authored in the `src.` versus the `projectFrom.` Prefix Form.
...
8. THE Object Mapper module SHALL NOT emit a design-time error or warning for an Expression Mapping in neither recognised Prefix Form, matching AutoMapper, which emits none either.

<Callout id="r2-decision" tone="decision" title="Parity was chosen over correctness, deliberately">

An unprefixed expression such as `Amount > 0 ? Amount : 0` does not work in AutoMapper — it prepends its source parameter to the whole string and leaves the second identifier unresolved — and Mapperly has no free-form expression support at all, so neither provider is a precedent for making it work. Resolving bare identifiers against the mapped entity's members would have made this module strictly better than both, but it would also mean a migrated model generates something AutoMapper never generated. For a drop-in replacement, generating **the same thing** matters more than generating the better thing. The cost is that bare and constant expressions stay broken (R2.4, and the last two rows of the table above); the benefit is that no existing mapping changes value on the swap.

Because those forms do not compile, neither Flavour Application models one — Mapping Shape 21 covers the two recognised Prefix Forms only.
...
3. **Out of scope for this spec.** Cursor-paged Call Sites are excluded — see the "Cursor paging is out of scope" callout below. This criterion is not verified and SHALL NOT be judged against.
4. IF an offset-paged query matches zero entities, THEN THE generated Call Site SHALL return an empty page with its metadata intact and SHALL NOT throw.
...
<Callout id="r4-cursor-out-of-scope" tone="warning" title="Cursor paging is out of scope">

**R4.3, and the cursor-paged clause of R8.9, are excluded from this spec's contract** by user decision. They are neither implemented nor verified here, and a verify pass SHALL NOT record a gap against them.

The blocker is upstream of the Object Mapper and unrelated to it. `Intent.Modules.EntityFrameworkCore` implements no cursor paging at all: `DbContextDataAccessProvider.FindAllAsync(queryMapping, pageSize, cursorToken, ...)` returns an empty statement under the comment "for now cursor based is not implemented with EF", and the module contains no cursor-paged repository method. The only module fulfilling `TemplateRoles.Application.Common.CursorPagedList` is `Intent.Modules.Azure.TableStorage`, a persistence provider whose repository templates would compete with EF Core's for the same roles. Modelling a cursor-paged query in either Flavour Application therefore generates a handler calling a repository method that does not exist, and the solution does not build.

`ObjectMappingMappingStrategy.ImplementPagedMappingStatement` is written generically over its `mappingMethod` argument, so it would emit the cursor form unchanged once EF Core gains cursor support. Closing this properly is a separate piece of work in `Intent.Modules.EntityFrameworkCore`.

**Offset paging is unaffected and fully in scope** — R4.1, R4.2, R4.4, R4.5 and the offset-paged clause of R8.9 are verified as written.
...
<Table
  ...
  rows={[
    ...
    ["20", "Nullable collection field — `List<T>?`", "Projection materialized to a list, yielding `null` rather than an empty list when the source collection is absent"],
    ["21", "Expression Mapping in the `src.` and `projectFrom.` Prefix Forms", "Per R2 — both forms produce byte-identical output. Unprefixed expressions are excluded: they do not compile, by parity with AutoMapper"]
  ]}
/>
...
2. WHEN a DTO field is a collection, THE Object Mapper module SHALL materialize the projection to a `List` of that field's element type — the only collection type a DTO field can take, since the Dtos module renders every collection field as a list.
3. WHEN a source collection is null at runtime AND the DTO collection field is non-nullable, THE generated Mapping Method SHALL assign an empty list and SHALL NOT throw.
...
8. WHEN a source collection is null at runtime AND the DTO collection field is nullable, THE generated Mapping Method SHALL assign `null` rather than an empty list.
...
7. EACH Flavour Application SHALL contain zero merge-managed generated files, **within the surface this module owns** — the Mapping Extension Classes and the query-handler Call Sites — whose content a full regeneration under R8.6 would not reproduce. *(Exception: a Domain Entity Operation body is merge-managed by design — the Domain designer cannot model a C# body, so the template emits a `NotImplementedException` stub for the developer to fill in. `Order.GetDisplayLabel()` is such a body: it exists purely as the test fixture Mapping Shape 13 maps through, is not module-generated output, and SHALL NOT be judged against this criterion.)*
...
9. EACH Flavour Application SHALL exercise at least one offset-paged query end to end, asserting both a populated page and an empty page. *(The cursor-paged half of this criterion is out of scope — see the "Cursor paging is out of scope" callout under Requirement 4 — and SHALL NOT be judged against.)*
...
<Checklist
  ...
  items={[
    ...
    { id: "a6", label: "Collections are always a list", note: "The Dtos module renders every collection DTO field as `List<T>` via a hardcoded collection formatter, so array, `ICollection` and `IEnumerable` targets cannot be modelled and are not catered for. Only the nullable-versus-non-nullable distinction varies." },
