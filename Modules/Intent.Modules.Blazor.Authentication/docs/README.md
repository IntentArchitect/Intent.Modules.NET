...
When `PersistentAuthenticationStateProvider.RequestAccessToken` decides the access token is missing or expired, it returns an `AccessTokenResult` with status `RequiresRedirect`. The framework surfaces this to calling code as a thrown `AccessTokenNotAvailableException` — you will typically see it wherever your generated HTTP client code calls a `Secured` API endpoint.

**Do not** catch that exception and then force a reload of the current page, e.g.:
...
`NavigationManager.Uri` is not guaranteed to have updated synchronously after `Redirect()`'s JS interop call returns, so `Navigation.Uri` can still read as the _current, still-expired_ page. The result looks like a brief flicker rather than a navigation — it is actually the same page reloading itself, over and over, because the API call fails with the same exception every time.

The correct pattern is to let `Redirect()` do the navigating on its own, without also reading `Navigation.Uri` afterwards:
...
- **A refresh token is only issued when `offline_access` is in `DefaultScopes` _and_ the identity provider's client has offline access enabled** (`AllowOfflineAccess = true` on IdentityServer). Its absence is a supported configuration — the generated code tolerates a token response with no `refresh_token`.
