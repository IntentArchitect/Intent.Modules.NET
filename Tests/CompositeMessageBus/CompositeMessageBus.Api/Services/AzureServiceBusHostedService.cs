...
namespace CompositeMessageBus.Api.Services
{
    public class AzureServiceBusHostedService : BackgroundService, IAsyncDisposable
    {
        ...
        private async Task ProcessMessageAsync(ProcessMessageEventArgs args, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var daprStateStoreUnitOfWork = scopedServiceProvider.GetRequiredService<IDaprStateStoreUnitOfWork>();
                ...
                await daprStateStoreUnitOfWork.SaveChangesAsync(cancellationToken);
