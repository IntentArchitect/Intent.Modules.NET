...
{
    public class MapAggChildMapping : IMongoMappingConfiguration<MapAggChild>
    {
        public string CollectionName => "MapAggChildren";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(MapAggChild)))
            {
                BsonClassMap.RegisterClassMap<MapAggChild>(
                    mapping =>
                    {
                        mapping.AutoMap();
