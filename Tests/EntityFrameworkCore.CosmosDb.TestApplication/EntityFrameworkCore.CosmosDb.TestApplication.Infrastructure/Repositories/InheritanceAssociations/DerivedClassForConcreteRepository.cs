...
namespace EntityFrameworkCore.CosmosDb.TestApplication.Infrastructure.Repositories.InheritanceAssociations
{
    ...
    {
        public DerivedClassForConcreteRepository(ApplicationDbContext dbContext, IMapper mapper) : base(dbContext, mapper)
        {
        }
        ...
        public async Task<DerivedClassForConcrete?> FindByIdAsync(
            Guid id,
            Func<IQueryable<DerivedClassForConcrete>, IQueryable<DerivedClassForConcrete>> queryOptions,
            CancellationToken cancellationToken = default)
        {
            return await FindAsync(x => x.Id == id, queryOptions, cancellationToken);
        }

        public async Task<List<DerivedClassForConcrete>> FindByIdsAsync(
            Guid[] ids,
            CancellationToken cancellationToken = default)
        {
