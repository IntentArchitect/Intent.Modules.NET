    ...
    public class OnUninstallMigration : IModuleOnUninstallMigration
    {
        ...
        public void OnUninstall()
        {
            ...
            foreach (var package in packages)
            {
                ...
                var integrationTestProject = package.GetElementsOfType(CSharpProjectSpecializationId).FirstOrDefault(p => p.Name == $"{applicationProjectName}.IntegrationTests");

                if (integrationTestProject is null)
                {
                    return;
                }

                var parentFolderId = integrationTestProject?.ParentFolderId;
                bool savePackage = false;
                if (integrationTestProject != null)
                {
                    foreach (var element in integrationTestProject.ChildElements.ToList())
                    {
                        integrationTestProject.ChildElements.Remove(element);
                    }

                    package.Classes.Remove(integrationTestProject);
