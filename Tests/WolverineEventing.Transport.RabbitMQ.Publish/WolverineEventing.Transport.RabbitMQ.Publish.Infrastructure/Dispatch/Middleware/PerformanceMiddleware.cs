...
namespace WolverineEventing.Transport.RabbitMQ.Publish.Infrastructure.Dispatch.Middleware
{
    public class PerformanceMiddleware
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogWarning("WolverineEventing.Transport.RabbitMQ.Publish Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName} {@Request}", requestName, stopwatch.ElapsedMilliseconds, user?.Id, user?.Name, envelope.Message);
            }
            else
            {
                logger.LogWarning("WolverineEventing.Transport.RabbitMQ.Publish Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName}", requestName, stopwatch.ElapsedMilliseconds, user?.Id, user?.Name);
            }
        }
    }
}