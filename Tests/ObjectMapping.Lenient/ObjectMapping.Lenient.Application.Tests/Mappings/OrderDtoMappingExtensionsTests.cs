using ObjectMapping.Lenient.Application.Orders;
using ObjectMapping.Lenient.Domain;
using ObjectMapping.Lenient.Domain.Entities;
using Xunit;

namespace ObjectMapping.Lenient.Application.Tests.Mappings
{
    /// <summary>
    /// One assertion per Mapping Shape (1-21) of the Object Mapper Golden Sample, plus the Lenient
    /// runtime contract (R1.7). Every value is distinct and non-default so a wrong mapping expression
    ...
    public class OrderDtoMappingExtensionsTests
    {
        ...
        [Fact]
        public void MapToOrderDto_DoesNotThrow_WhenNullableHopIntoNonNullableTargetIsNull_Coupon()
        {
            // Arrange
            var order = CreateFullyPopulatedOrder();
            order.Coupon = null;
        ...
        [Fact]
        public void MapToOrderDto_YieldsDefaultsForTheGuardedFields_WhenCouponIsNull()
        {
            // Arrange
            var order = CreateFullyPopulatedOrder();
            order.Coupon = null;
            ...
            // Assert
            Assert.Equal(Guid.Empty, result.CouponId);
            Assert.Equal(0, result.CouponPercentOff);
            Assert.Equal(default, result.CouponKind);
        }

        [Fact]
        public void MapToOrderDto_PopulatesEveryOtherField_WhenCouponIsNull()
        {
            // Arrange
            var order = CreateFullyPopulatedOrder();
            order.Coupon = null;
        ...
        [Fact]
        public void MapToOrderDto_DoesNotThrow_WhenNullableHopIntoNonNullableTargetIsNull_Address()
        {
            // Arrange
            var order = CreateFullyPopulatedOrder();
        ...
        [Fact]
        public void MapToOrderDto_YieldsDefaultForTheGuardedField_WhenAddressIsNull()
        {
            // Arrange
            var order = CreateFullyPopulatedOrder();
            order.Customer.Address = null;
            ...
            // Assert
            Assert.Null(result.CustomerCity);
        }

        [Fact]
        public void MapToOrderDto_PopulatesEveryOtherField_WhenAddressIsNull()
        {
            // Arrange
            var order = CreateFullyPopulatedOrder();
            order.Customer.Address = null;
