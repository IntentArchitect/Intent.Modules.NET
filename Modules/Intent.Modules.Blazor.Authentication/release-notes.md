...
- Fixed: When an access token expired, `PersistentAuthenticationStateProvider.RequestAccessToken` correctly decided to redirect to the login page, but passed `null` for the `AccessTokenResult` constructor's `InteractiveRequestOptions` argument. `AccessTokenNotAvailableException.Redirect()` only follows the login URL when that argument is also populated, so it silently failed to navigate instead — the user saw a blank error with no redirect. The result now carries a populated `InteractiveRequestOptions`, which is what `Redirect()` actually requires.
