...
public class EfCoreKeyColumnPropertyStatement : CSharpStatement
{
    ...
    {
        return attribute.HasColumn() || HasSpecialPrimaryKeyConfiguration(attribute);
    }

    private static bool HasSpecialPrimaryKeyConfiguration(AttributeModel attribute)
    {
        return attribute.GetPrimaryKey() is not null &&
               (attribute.GetPrimaryKey().DataSource().IsAutoGenerated() ||
                ...
                (attribute.GetPrimaryKey().DataSource().IsDefault() && NonConventionalOrdinalPrimaryKeyRequiresConfiguration(attribute)));
    }
