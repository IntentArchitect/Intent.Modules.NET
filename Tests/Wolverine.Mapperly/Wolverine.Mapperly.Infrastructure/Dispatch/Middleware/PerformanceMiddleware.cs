...
namespace Wolverine.Mapperly.Infrastructure.Dispatch.Middleware
{
    public class PerformanceMiddleware
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                logger.LogWarning("Wolverine.Mapperly Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName} {@Request}", requestName, stopwatch.ElapsedMilliseconds, user?.Id, user?.Name, envelope.Message);
            }
            else
            {
                logger.LogWarning("Wolverine.Mapperly Long Running Request: {Name} ({ElapsedMilliseconds} milliseconds) {@UserId} {@UserName}", requestName, stopwatch.ElapsedMilliseconds, user?.Id, user?.Name);
            }
        }
    }
}