...
public class DeleteWithReturnDtoImplementationStrategy : IImplementationStrategy
{
    ...
    public void ApplyStrategy(OperationModel operationModel)
    {
        ...
        codeLines.Add(
            $@"var {existingEntityVariableName} ={(operationModel.IsAsync() ? " await" : "")} {repositoryFieldName}.FindById{(operationModel.IsAsync() ? "Async" : "")}({operationModel.Parameters.Single().Name.ToCamelCase()}, cancellationToken);");
        ...
        codeLines.Add($"{repositoryFieldName}.Remove({existingEntityVariableName});");
        codeLines.Add($@"return {existingEntityVariableName}.MapTo{dtoType}(_mapper);");
