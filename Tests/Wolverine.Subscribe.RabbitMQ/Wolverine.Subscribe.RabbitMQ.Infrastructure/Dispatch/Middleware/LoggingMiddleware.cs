...
namespace Wolverine.Subscribe.RabbitMQ.Infrastructure.Dispatch.Middleware
{
    public class LoggingMiddleware
    {
        ...
        private async Task LogAsync(object request, ILogger logger, ICurrentUserService currentUserService)
        {
            ...
            {
                logger.LogInformation("Wolverine.Subscribe.RabbitMQ Request: {Name} {@UserId} {@UserName} {@Request}", requestName, user?.Id, user?.Name, request);
            }
            else
            {
                logger.LogInformation("Wolverine.Subscribe.RabbitMQ Request: {Name} {@UserId} {@UserName}", requestName, user?.Id, user?.Name);
            }
        }
    }
}