...
namespace IntegrationTesting.Tests.IntegrationTests.HttpClients
{
    public class NoReturnsHttpClient : INoReturnsService
    {
        ...
        public NoReturnsHttpClient(HttpClient httpClient)
        {
        ...
        public async Task UpdateNoReturnAsync(
            Guid id,
            UpdateNoReturnCommand command,
        ...
        public async Task<NoReturnDto> GetNoReturnByIdAsync(Guid id, CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    return (await JsonSerializer.DeserializeAsync<NoReturnDto>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
        ...
        public async Task<List<NoReturnDto>> GetNoReturnsAsync(CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<List<NoReturnDto>>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
