...
- Improvement: Each bundled AI skill's sample files (e.g. `add-entity-dialog-sample.razor`/`.razor.cs` for `blazor-dialog-adding-entity`) are no longer overwritten once that skill's own `SKILL.md` has been hand-edited, on the assumption you have taken over maintenance of the whole skill.
