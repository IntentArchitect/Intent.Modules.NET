...
namespace FluentValidationTest.Application.ValidationScenarios.DomainMapped.UpdateNumericConstrainedEntity
{
    ...
    {
        public UpdateNumericConstrainedEntityCommand(Guid id, int age, int percentage, double score, decimal price, int? optionalThreshold, float exclusiveMinInclusiveMaxFloat, double inclusiveMinExclusiveMaxDouble, decimal exclusiveMinExclusiveMaxDecimal, float onlyMinExclusiveFloat, double onlyMaxExclusiveDouble, float inclusiveMinInclusiveMaxFloat, decimal negativeRangeDecimal, double narrowRangeDouble)
        {
            Id = id;
            Age = age;
