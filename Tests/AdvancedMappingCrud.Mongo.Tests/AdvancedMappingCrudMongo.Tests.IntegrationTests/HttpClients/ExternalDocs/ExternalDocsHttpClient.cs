...
namespace AdvancedMappingCrudMongo.Tests.IntegrationTests.HttpClients.ExternalDocs
{
    public class ExternalDocsHttpClient : IExternalDocsService
    {
        ...
        public ExternalDocsHttpClient(HttpClient httpClient)
        {
        ...
        public async Task<long> CreateExternalDocAsync(
            CreateExternalDocCommand command,
            CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                {
                    var wrappedObj = (await JsonSerializer.DeserializeAsync<JsonResponse<long>>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
        ...
        public async Task UpdateExternalDocAsync(
            long id,
            UpdateExternalDocCommand command,
            CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/external-docs/{id}";
        ...
        public async Task<ExternalDocDto> GetExternalDocByIdAsync(long id, CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<ExternalDocDto>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
        ...
        public async Task<List<ExternalDocDto>> GetExternalDocsAsync(CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<List<ExternalDocDto>>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
