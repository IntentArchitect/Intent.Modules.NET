...
{
    public class J_MultipleDependentMapping : IMongoMappingConfiguration<J_MultipleDependent>
    {
        public string CollectionName => "J_MultipleDependents";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(J_MultipleDependent)))
            {
                BsonClassMap.RegisterClassMap<J_MultipleDependent>(
                    mapping =>
                    {
                        mapping.AutoMap();
