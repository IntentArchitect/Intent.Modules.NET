...
namespace Lambda
{
    public class ClientCreatedEventConsumer
    {
        ...
        public ClientCreatedEventConsumer(ILogger<ClientCreatedEventConsumer> logger,
        ...
        public async Task ProcessAsync(SQSEvent sqsEvent, ILambdaContext context)
        {
            ...
            foreach (var record in sqsEvent.Records)
            {
                ...
                {
                    _logger.LogError(ex, "Error processing ClientCreatedEventConsumer message with ID {MessageId}", record.MessageId);
                    throw;
                }
            }
        }
