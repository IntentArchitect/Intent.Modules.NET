...
{
    public class GetCustomerThing
    {
        private readonly IMediator _mediator;

        public GetCustomerThing(IMediator mediator)
        {
            _mediator = mediator ?? throw new ArgumentNullException(nameof(mediator));
        }

        [FunctionName("Customers_GetCustomerThing")]
        [OpenApiOperation("GetCustomerThing", tags: new[] { "Customers" }, Description = "Check CustomerId route parameter generates correctly")]
        ...
        public async Task<IActionResult> Run(
            [HttpTrigger(AuthorizationLevel.Function, "get", Route = "customers/{customerId}/thing")] HttpRequest req,
            Guid customerId,
            CancellationToken cancellationToken)
        {
            try
            {
                var result = await _mediator.Send(new Application.Customers.GetCustomerThing.GetCustomerThing(customerId: customerId), cancellationToken);
