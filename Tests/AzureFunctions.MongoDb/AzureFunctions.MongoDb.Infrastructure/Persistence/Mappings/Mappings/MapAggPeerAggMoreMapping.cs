...
{
    public class MapAggPeerAggMoreMapping : IMongoMappingConfiguration<MapAggPeerAggMore>
    {
        public string CollectionName => "MapAggPeerAggMores";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(MapAggPeerAggMore)))
            {
                BsonClassMap.RegisterClassMap<MapAggPeerAggMore>(
                    mapping =>
                    {
                        mapping.AutoMap();
