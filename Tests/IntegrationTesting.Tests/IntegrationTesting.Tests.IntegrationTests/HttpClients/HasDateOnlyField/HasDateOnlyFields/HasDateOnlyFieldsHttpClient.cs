...
{
    public class HasDateOnlyFieldsHttpClient : IHasDateOnlyFieldsService
    {
        private readonly JsonSerializerOptions _serializerOptions;
        private readonly HttpClient _httpClient;

        public HasDateOnlyFieldsHttpClient(HttpClient httpClient)
        {
        ...
        public async Task<Guid> CreateHasDateOnlyFieldAsync(
            CreateHasDateOnlyFieldCommand command,
        ...
        public async Task UpdateHasDateOnlyFieldAsync(
            Guid id,
            UpdateHasDateOnlyFieldCommand command,
            CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/has-date-only-field/has-date-only-fields/{id}";
        ...
        public async Task<HasDateOnlyFieldDto> GetHasDateOnlyFieldByIdAsync(
            Guid id,
            CancellationToken cancellationToken = default)
        {
            var relativeUri = $"api/has-date-only-field/has-date-only-fields/{id}";
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<HasDateOnlyFieldDto>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
        ...
        public async Task<List<HasDateOnlyFieldDto>> GetHasDateOnlyFieldsAsync(CancellationToken cancellationToken = default)
        {
            ...
            using (var response = await _httpClient.SendAsync(httpRequest, HttpCompletionOption.ResponseHeadersRead, cancellationToken).ConfigureAwait(false))
            {
                ...
                using (var contentStream = await response.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false))
                {
                    return (await JsonSerializer.DeserializeAsync<List<HasDateOnlyFieldDto>>(contentStream, _serializerOptions, cancellationToken).ConfigureAwait(false))!;
                }
            }
        }
