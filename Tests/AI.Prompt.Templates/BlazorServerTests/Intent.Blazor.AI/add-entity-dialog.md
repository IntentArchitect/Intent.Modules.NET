...
- Implement two top-level methods for the template to use:
	- `Save()` / `SaveAsync()`:
		- calls the existing service method (e.g. `CreateEntity()` / `UpdateEntity()` logic)
...
- If a method like `CreateEntity()` or `UpdateEntity()` already exists and calls the backend, either:
