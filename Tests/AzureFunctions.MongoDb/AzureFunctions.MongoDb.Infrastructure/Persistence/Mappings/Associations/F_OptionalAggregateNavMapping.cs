...
{
    public class F_OptionalAggregateNavMapping : IMongoMappingConfiguration<F_OptionalAggregateNav>
    {
        public string CollectionName => "F_OptionalAggregateNavs";

        public void RegisterCollectionMap()
        {
            if (!BsonClassMap.IsClassMapRegistered(typeof(F_OptionalAggregateNav)))
            {
                BsonClassMap.RegisterClassMap<F_OptionalAggregateNav>(
                    mapping =>
                    {
                        mapping.AutoMap();
