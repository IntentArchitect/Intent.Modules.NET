...
namespace CosmosDB.Infrastructure.Persistence.Documents.Throughput
{
    internal class ManualDocument : IManualDocument, ICosmosDBDocument<Manual, ManualDocument>
    {
        ...
        public ManualDocument PopulateFromEntity(Manual entity, Func<string, string?> getEtag)
        {
            Id = entity.Id;
        ...
        public static ManualDocument? FromEntity(Manual? entity, Func<string, string?> getEtag)
        {
            if (entity is null)
            {
                return null;
