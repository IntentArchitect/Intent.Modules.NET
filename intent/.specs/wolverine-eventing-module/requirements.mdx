...
- **UJ-5. Nadia proves a failing message cannot lose data.**
  ...
  - **Path:** She turns Transactional Outbox to Durable in Sales and re-runs the Software Factory; makes the Warehouse Integration Event Handler throw on purpose and watches the re-deliveries; then kills the Sales process mid-transaction to see whether an order can commit without its messages, or messages dispatch without their order.
...
- One installable module covering all four Transports and the Durable Transactional Outbox — not the root-plus-companion split the MassTransit modules use.
...
- Runtime proof: a publisher/subscriber application pair exercised end to end on RabbitMQ. The remaining three Transports are proven by generating and compiling their configuration, with runtime verification done by hand outside this scope.
...
- **SM-2**: Transport coverage — the number of the four Transports whose configuration generates and compiles, of which RabbitMQ is additionally demonstrated publishing and consuming across two applications at runtime. Target: four of four generating, one of four at runtime. Validates R2, R3, R4.
...
3. WHEN Transactional Outbox is Durable, THE Software Factory SHALL generate a durable outbox and inbox backed by the application's Entity Framework Core database, such that a message published inside a transaction is dispatched if and only if that transaction commits.
...
5. WHEN Transactional Outbox is Durable, THE generated inbox SHALL cause a message re-delivered by the Error Handling Policy to be processed by its Integration Event Handler at most once.
...
1. THE Wolverine eventing module SHALL be installable as one module that covers all four Transports and the Durable Transactional Outbox, with no companion module required.
...
5. WHEN the Wolverine eventing module is uninstalled and the Software Factory is re-run, THE Software Factory SHALL remove the Consumers and the Wolverine Host Configuration registrations it added, and SHALL leave hand-written Integration Event Handler classes in place. THE `appsettings.json` sections the module added SHALL be left in place and THE module documentation SHALL name the sections to delete by hand, for the same reason given in R2.4.
...
6. THE Reference Solution SHALL be exercised at full R17 test depth on one non-Local Transport, and SHALL additionally prove, for each remaining Transport, only that the transport registration call and the error-handling and dead-letter shape are generated and compile. Runtime verification of the remaining Transports is explicitly out of scope for this requirement and is done by hand.
...
10. IF a Transport's generation-and-compile check under R16.6 reveals a transport registration or error-handling shape differing from the fully-exercised Transport, THEN that Transport SHALL be promoted to full R17 test depth in the Reference Solution before its template branch is written.
...
<Checklist
  ...
  items={[
    ...
    { id: "a9", label: "R2.5's claim that Local loses messages on process stop is unverified where Transactional Outbox is Durable", note: "Wolverine has durable local queues backed by the database, so Durable plus Local may well give a restart-surviving in-process queue rather than the loss R2.5 describes. The combination is deliberately NOT forbidden — that would invent a restriction Wolverine does not have. It is left as written and checked once against the pinned WolverineFx version before the durability branch is built; if the check contradicts R2.5, R2.5 is amended then rather than being softened now on an expectation." },
    { id: "a10", label: "Only RabbitMQ is proven at runtime; Azure Service Bus, Amazon SQS and Local are proven by generating and compiling", note: "R16.6, SM-2 and the MVP scope all say this explicitly rather than promising four-transport runtime proof. Runtime verification of the other three is done by hand, outside this specification. The accepted consequence is that a transport-specific runtime fault which still compiles — a wrong dead-letter registration, say — is not caught by any automated gate here; R16.10 is the route to promoting a Transport to full depth if the hand pass finds one." },
    { id: "a11", label: "R6.5's at-most-once inbox guarantee is verified by generation and inspection, not by a runtime test", note: "R17's four tests cover publish, send, error handling and the outbox side of the transaction boundary; none asserts that a re-delivered message reaches its handler only once. Adding a fifth test was considered and deliberately not taken — the criterion stays because the guarantee is real and comes from Wolverine's own inbox, but nothing in R17 will fail if it regresses." },
    { id: "a12", label: "Documentation is the module's own `docs/README.md`, not modelled help topics", note: "Every requirement that says \"module documentation\" or \"migration section\" — R1.6, R2.4, R2.5, R6.7, R7.4, R10.3, R13.1 to R13.5, R14.5 — is satisfied by a README section, matching how every other eventing module in this repository carries its prose. No `Module Documentation` element is modelled, so none of those criteria may be verified by pointing at a generated topic." },
