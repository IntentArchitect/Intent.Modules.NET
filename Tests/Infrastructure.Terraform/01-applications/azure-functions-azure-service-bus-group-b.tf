...
resource "azurerm_windows_function_app" "azure-functions-azure-service-bus-group-b_function_app" {
  ...
  app_settings = {
    ...
    "AzureServiceBus:ClientCreatedSubscription"               = azurerm_servicebus_subscription.client_created.name
    "AzureServiceBus:CreateOrgIntegrationCommand"             = azurerm_servicebus_topic.create_org_integration_command.name
    "AzureServiceBus:CreateOrgIntegrationCommandSubscription" = azurerm_servicebus_subscription.create_org_integration_command.name
  }
}
