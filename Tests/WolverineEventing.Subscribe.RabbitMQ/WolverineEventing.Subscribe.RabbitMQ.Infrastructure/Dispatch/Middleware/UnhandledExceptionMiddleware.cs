...
namespace WolverineEventing.Subscribe.RabbitMQ.Infrastructure.Dispatch.Middleware
{
    public class UnhandledExceptionMiddleware
    {
        ...
        private void LogException(Exception exception, object? request, ILogger logger)
        {
            ...
            {
                logger.LogError(exception, "WolverineEventing.Subscribe.RabbitMQ Request: Unhandled Exception for Request {Name} {@Request}", requestName, request);
            }
            else
            {
                logger.LogError(exception, "WolverineEventing.Subscribe.RabbitMQ Request: Unhandled Exception for Request {Name}", requestName);
            }
        }
    }
}