                ...
                switch (mappedPropertyElement?.Element?.SpecializationTypeId)
                {
                    ...
                    case AssociationTargetEndModel.SpecializationTypeId:
                    {
                        ...
                        if (association.Multiplicity is Multiplicity.One or Multiplicity.ZeroToOne)
                        {
                            ...
                            {
                                codeLines.Add($"{propertyModel.Name.ToPascalCase()} = {GetCreateMethodName(targetType, attributeName)}({entityVarExpr}{propertyModel.Name.ToPascalCase()}),");
                            }
                        }
                        else
                        {
                            codeLines.Add(
                                $"{propertyModel.Name.ToPascalCase()} = {entityVarExpr}{propertyModel.Name.ToPascalCase()}{(association.IsNullable ? "?" : "")}.Select({GetCreateMethodName(targetType, attributeName)}).ToList(),");
                        }
        
                        var @class = this.CSharpFile.Classes.First();
                        @class.AddMethod(this.GetTypeName((IElement)propertyModel.TypeReference.Element),
                            GetCreateMethodName(targetType, attributeName),
                            method => method.Private()
                                .Static()
                                .AddAttribute(CSharpIntentManagedAttribute.Fully())
                                .AddParameter(this.GetTypeName(targetType.InternalElement), "message")
                                .AddStatement($"return new {this.GetTypeName((IElement)propertyModel.TypeReference.Element)}")
                                .AddStatement(new CSharpStatementBlock()
                                    .AddStatements(GetMessagePropertyAssignments("message", targetType,
