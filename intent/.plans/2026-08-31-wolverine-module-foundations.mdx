...
| 2 | Generated `WolverineEventingConfiguration` has body text at column 0 | `Intent.Eventing.Wolverine` | Folded into #3 — the file is retired and rebuilt with builder constructs |
| 3 | Two modules reach into `Program.cs` directly, and the eventing side's method names leak the transport choice | `Intent.Wolverine.Common` (new owner), `Intent.Application.Wolverine` + `Intent.Eventing.Wolverine` (contributors) | One seeded `WolverineConfiguration.Configure`; each module contributes a private method |
...
> **This fix lands as a side effect of Point 3, not as its own patch.** `WolverineEventingConfigurationTemplatePartial.cs`
> is retired as part of Point 3's redesign (see below) and its `UseRabbitMq` lambda is rebuilt from proper
> nested builder statements rather than a raw string — so the indentation defect has nowhere to reappear,
> rather than being patched in place on a file about to be deleted.

---

### 3. Two modules reach into `Program.cs` directly, and the eventing side's method names leak the transport

#### The issue

This was originally scoped as "two disconnected configuration classes" — that part turned out to already
be fixed by the time this plan was executed: `Intent.Wolverine.Common` seeds one shared
`builder.Host.UseWolverine(opts => { ... })` lambda, and both `Intent.Application.Wolverine`'s
`WolverineConfiguration.Configure(opts)` and `Intent.Eventing.Wolverine`'s
`WolverineEventingConfiguration.ConfigureRabbitMq(opts, configuration)` already append their own
statement into it, ordered by `Order` (10/20). Verified directly in generated `Program.cs` across five
different test apps before touching anything further — there is no disconnect left to fix.

Two smaller things are still worth fixing:

- **Two modules still reach into `Program.cs` directly** — a file neither of them owns. That should stay
exceptional, reserved for a module that genuinely needs the host builder itself, not the default shape
every Wolverine module reaches for.
- **The eventing side's method names carry the transport** (`ConfigureLocal`, `ConfigureRabbitMq`,
`ConfigureAzureServiceBus`, `ConfigureAmazonSqs`), so which transport is selected leaks into the public
shape of generated code instead of staying a body detail.
...
subgraph today["Today — two modules reach into Program.cs"]
    P1["Program.cs — UseWolverine lambda,<br/>seeded by Wolverine.Common"]:::existing
    C1["WolverineConfiguration.Configure(opts)<br/>Application.Wolverine, Order 10"]:::existing
    E1["WolverineEventingConfiguration.ConfigureRabbitMq(opts, config)<br/>Eventing.Wolverine, Order 20"]:::existing
    P1 --> C1
    P1 --> E1
end

subgraph target["Proposed — Program.cs touched once, by Common only"]
    P2["Program.cs — one line, forever:<br/>WolverineConfiguration.Configure(opts, configuration)"]:::existing
    C2["WolverineConfiguration<br/>Wolverine.Common, Api project"]:::added
    P2 --> C2
    C2 --> Q1["private ConfigureCqrs(opts)<br/>contributed by Application.Wolverine"]:::changed
    C2 --> Q2["private ConfigureEventing(opts, configuration)<br/>contributed by Eventing.Wolverine"]:::changed
    Q2 --> R1["private ConfigurePublishing / ConfigureListeners /<br/>Configure{Transport}Transport — Wolverine's own vocabulary"]:::changed
end
```

#### The proposed solution

`Intent.Wolverine.Common` gains a template that seeds a `WolverineConfiguration` class with one public,
near-empty method:
...
`Program.cs` calls this once, and never needs a second statement for a new Wolverine module to show up:

```csharp
builder.Host.UseWolverine(opts => WolverineConfiguration.Configure(opts, builder.Configuration));
```

Each contributing module finds this template and extends it the same way
`Intent.Eventing.MassTransit`'s `FinbuckleConfiguratorExtension` already finds and extends
`MassTransitConfigurationTemplate` — no new mechanism, just this codebase's existing pattern for one
module extending a class another module owns:

```csharp
var template = application.FindTemplateInstance<ICSharpFileBuilderTemplate>(
    TemplateDependency.OnTemplate(WolverineConfigurationTemplate.TemplateId));
...
template?.CSharpFile.OnBuild(file =>
{
    ...
    @class.AddMethod("void", "ConfigureCqrs", method => { /* this module's own logic */ });
    configureMethod.AddStatement("ConfigureCqrs(opts);");
});
```

`Intent.Application.Wolverine` contributes a private `ConfigureCqrs(WolverineOptions opts)` — its
existing `Configure` body, unchanged, just relocated. `Intent.Eventing.Wolverine` contributes a private
`ConfigureEventing(WolverineOptions opts, IConfiguration configuration)`, itself decomposed into further
private methods named after Wolverine's own vocabulary — its API return types are `PublishingExpression`
and `ListenerConfiguration`, and the broker plumbing is a "Transport" (`IRabbitMqTransportExpression`,
etc.):
...
private static IRabbitMqTransportExpression ConfigureRabbitMqTransport(WolverineOptions opts, IConfiguration configuration)
{
...
private static void ConfigureListeners(WolverineOptions opts, IRabbitMqTransportExpression transport)
{
    transport.BindExchange("order-created-event").ToQueue("subscribe-rabbitmq-order-created-event");
    opts.ListenToRabbitQueue("subscribe-rabbitmq-order-created-event");
    opts.Discovery.IncludeType<OrderCreatedEventHandler>();
}
```

- **`Configure{Transport}Transport` also folds in Point 2's indentation fix for free** — the
`UseRabbitMq` lambda is built from nested builder statements instead of a raw string, so there is no
raw multi-line literal left to mis-indent.
- **A seam with nothing to say is omitted, not emitted empty.** A publish-only app has no
`ConfigureListeners` method and no call to it — not an empty method contributed for symmetry.
- **`transport` is threaded through as a return value**, because RabbitMQ's exchange-to-queue binding
needs the same transport object `.AutoProvision()` was called on — it cannot be re-obtained from `opts`
later. Azure Service Bus and Amazon SQS don't need this thread at all: Azure's `ConfigureListeners`
calls `opts.ListenToAzureServiceBusQueue(...)` directly with no binding step, and Amazon SQS's fan-out
is expressed entirely on the publish side (`SubscribeSqsQueue(...)` chained onto `ConfigurePublishing`'s
rule), so it needs no `ConfigureListeners` method at all.
- **`ConfigureLocal` / `ConfigureRabbitMq` collapse into one name, `ConfigureEventing`**, called
identically regardless of transport. Which transport is selected now only decides which private
`Configure{Transport}Transport`/`ConfigureListeners` bodies get emitted inside it — never part of a name
`Program.cs` or another module can see.

> **Tradeoff — both `WolverineConfiguration.cs` (Application.Wolverine's) and
> `WolverineEventingConfiguration.cs` (Eventing.Wolverine's) are retired as their own templates,
> everywhere either exists today.** Their content becomes contributed methods inside
> `Api/Configuration/WolverineConfiguration.cs`, owned by `Intent.Wolverine.Common`. Regeneration reports
> this as a destructive removal in every app that had either file. Accepted because these are
> golden-sample test apps with no hand-written code in either file — a real consumer upgrading across
> this version needs it called out as a breaking change in the release notes, not a bullet.
...
| `Modules/Intent.Modules.Wolverine.Common/Templates/WolverineConfiguration/` | **added** | Point 3 — the shared configuration seed, this module's first template |
| `Modules/Intent.Modules.Application.Wolverine/Templates/WolverineConfiguration/` | **removed** | Point 3 — retired as its own template |
| `Modules/Intent.Modules.Application.Wolverine/FactoryExtensions/WolverineConfigurationContributionExtension.cs` | **added** | Point 3 — contributes `ConfigureCqrs` into Wolverine.Common's `WolverineConfiguration` |
| `Modules/Intent.Modules.Eventing.Wolverine/Templates/WolverineEventingConfiguration/` | **removed** | Points 2, 3 — retired as its own template |
| `Modules/Intent.Modules.Eventing.Wolverine/FactoryExtensions/WolverineEventingConfigurationContributionExtension.cs` | **added** | Points 2, 3, 5 — contributes `ConfigureEventing` (+ `ConfigurePublishing`/`ConfigureListeners`/`Configure{Transport}Transport`) into Wolverine.Common's `WolverineConfiguration`, plus the role/registry wiring from Point 5 |
...
4. **Point 3a** — add `WolverineConfiguration` to `Intent.Wolverine.Common`: the seed template, with one
    public `Configure(opts, configuration)` method, near-empty.
5. **Point 3b** — retire `Intent.Application.Wolverine`'s own `WolverineConfiguration` template; replace
    it with a factory extension contributing a private `ConfigureCqrs` method + one call statement into
    Common's class.
6. **Point 3c / Point 2** — retire `Intent.Eventing.Wolverine`'s `WolverineEventingConfiguration`
    template; replace it with a factory extension contributing a private `ConfigureEventing` method,
    decomposed into `ConfigurePublishing`/`ConfigureListeners`/`Configure{Transport}Transport`, built from
    proper builder constructs rather than raw strings — this is where Point 2's indentation defect is
    fixed, by construction rather than by patching a file about to be deleted.
...
9. **Point 5a** — add `FulfillsRole`, `AddWolverineEventingConfiguration`, the DI registration and the
    registry mapping to the new `WolverineEventingConfigurationContributionExtension`; honour
    `RequiresCompositeMessageBus()`.
...
11. **Regenerate every Wolverine app**, inspecting diffs via `get_file_diffs` before applying. Expect the
    destructive removal of `WolverineConfiguration.cs`/`WolverineEventingConfiguration.cs` wherever either
    exists today, replaced by the shared file.
...
- [ ] **One `WolverineConfiguration.cs` per app, nothing named `WolverineEventingConfiguration` anywhere**
        — `Program.cs` calls it exactly once; `ConfigureEventing` is present wherever Eventing.Wolverine is
        installed, `ConfigureCqrs` wherever Application.Wolverine is installed, and a seam with nothing to
        contribute (e.g. `ConfigureListeners` in a publish-only app) is absent, not present-but-empty.
- [ ] **Indentation** — read the generated configuration file: the `UseRabbitMq` lambda body inside
        `Configure{Transport}Transport` is indented inside its braces.
...
| --- | --- | --- |
| Owner of the shared configuration class | `Intent.Wolverine.Common` | It is installed wherever any Wolverine module is, and had zero templates of its own, giving it a reason to exist |
| How contributors reach the shared class | Find-template + `OnBuild` + `AddMethod`/`AddStatement`, the same idiom `Intent.Eventing.MassTransit`'s `FinbuckleConfiguratorExtension` already uses on `MassTransitConfigurationTemplate` | Reuses an existing, proven cross-template pattern instead of inventing a new merging mechanism just for this class |
| `Program.cs` touch points | Only `Intent.Wolverine.Common`, once, forever | The two contributors reaching into a file neither owns should be exceptional, not the default shape |
| Eventing reached from the shared class | Private `ConfigureEventing` method | A second public entry point is what the consolidation exists to remove |
| Naming for eventing's private sub-methods | `ConfigurePublishing` / `ConfigureListeners` / `Configure{Transport}Transport` | Matches Wolverine's own API vocabulary (`PublishingExpression`, `ListenerConfiguration`, `IRabbitMqTransportExpression`) rather than inventing new terms |
| A seam with nothing to contribute | Omitted entirely — not emitted as an empty method | An empty method contributed for symmetry is dead code with no caller reading it |
| Sharing the transport object across sub-methods | Returned from `Configure{Transport}Transport`, passed as a parameter to `ConfigureListeners` | RabbitMQ's exchange-to-queue binding needs the exact transport object `.AutoProvision()` was called on; it cannot be re-obtained from `opts` later |
