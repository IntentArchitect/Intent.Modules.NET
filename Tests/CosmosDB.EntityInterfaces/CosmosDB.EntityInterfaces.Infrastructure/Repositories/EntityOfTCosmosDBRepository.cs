...
namespace CosmosDB.EntityInterfaces.Infrastructure.Repositories
{
    internal class EntityOfTCosmosDBRepository<T> : CosmosDBRepositoryBase<IEntityOfT<T>, EntityOfT<T>, EntityOfTDocument<T>, IEntityOfTDocument<T>>, IEntityOfTRepository<T>
    {
        ...
        public async Task<IEntityOfT<T>?> FindByIdAsync(string id, CancellationToken cancellationToken = default) => await base.FindByIdAsync(id: id, cancellationToken: cancellationToken);

        public override string GetId(IEntityOfT<T> entity) => entity.Id;
    }
}